...
export default function ProviderSelectItems() {
  ...
  const requestNewItem = async () => {
    if (!request.proposedItemName.trim()) return alert("Enter proposed item name.");
  ...
  return (
    <div style={styles.page}>
      <div style={styles.header}>
        <div>
          <h1 style={styles.title}>Select Items & Add Rates</h1>
          <p style={styles.sub}>Provider details come from your login profile. You only select master items and enter rates.</p>
        </div>
      ...
      <div style={styles.toolbar}>
        <button style={styles.button} onClick={() => setSelected({ ...selected, ...Object.fromEntries(visibleItems.map((x) => [x.masterItemCode, true])) })}>Select visible</button>
        <button style={styles.secondaryButton} onClick={() => setSelected({})}>Unselect all</button>
        <button style={styles.successButton} onClick={submitSelected}>Bulk submit selected</button>
      </div>
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={item.masterItemCode}>
                ...
                <td style={styles.td}><img src={item.imageUrl} alt={item.itemName} style={styles.thumb} /></td>
      ...
      <div style={styles.requestBox}>
        <h2 style={styles.sectionTitle}>Request New Item</h2>
        <div style={styles.requestGrid}>
          <input style={styles.input} placeholder="Proposed item name" value={request.proposedItemName} onChange={(e) => setRequest({ ...request, proposedItemName: e.target.value })} />
          ...
          <input style={styles.input} placeholder="Image/photo URL" value={request.imageUrl} onChange={(e) => setRequest({ ...request, imageUrl: e.target.value })} />
...
const styles: Record<string, React.CSSProperties> = {
  ...
  filters: { maxWidth: 1280, margin: "0 auto 12px", display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr 1fr", gap: 8 },
