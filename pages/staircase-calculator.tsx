...
const router = useRouter();
  ...
  const agg = getMasterRate(["aggregate", "20mm aggregate", "ca1"], 50);
  const steel = getMasterRate(["steel", "tmt", "rebar"], 68);
  const binding = getMasterRate(["binding wire"], 80);
  const labour = getMasterRate(["staircase labour", "rcc labour"], 1000, ["bm_labour_rates", "bm_service_rates"]);
  const granite = getMasterRate(["granite"], 120);
  const marble = getMasterRate(["marble"], 150);
  const tile = getMasterRate(["tile", "tiles"], 45);
  const ips = getMasterRate(["ips", "ips finish"], 55);
  const ssRail = getMasterRate(["ss railing"], 3500);
  const msRail = getMasterRate(["ms railing"], 1800);
  const glassRail = getMasterRate(["glass railing"], 4500);
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={{ textAlign: "center", marginTop: 15 }}>
        ...
        {result && <button style={{ ...styles.btn, background: "#28a745" }} onClick={() => checkAndRun('calculator_export', 'staircase-calculator', exportExcel)}>📊 Excel</button>}
