...
const styles = {
  ...
  header: { backgroundColor: '#6d4c41', padding: '12px', borderRadius: '8px', marginBottom: '15px', color: 'white', display: 'flex', alignItems: 'center', gap: '10px' },
  ...
  sectionTitle: { backgroundColor: '#e8f4f8', color: '#6d4c41', padding: '8px', borderRadius: '6px', marginBottom: '12px', fontSize: '14px', fontWeight: 'bold', textAlign: 'center', border: '1px solid #cce5ed' },
  ...
  subSection: { backgroundColor: '#f0f0f0', padding: '8px', borderRadius: '6px', border: '1px solid #ddd' },
  subTitle: { fontSize: '11px', fontWeight: 'bold', color: '#6d4c41', marginBottom: '8px', textAlign: 'center' },
  ...
  th: { backgroundColor: '#6d4c41', color: 'white', padding: '8px', textAlign: 'left' },
...
export default function RetainingWallPage() {
  ...
  // Toe/Heel Reinforcement
  const [toeBarDia, setToeBarDia] = useState(12);
  const [toeBarSpacing, setToeBarSpacing] = useState(150);
  ...
  const calculateResults = () => {
    ...
    let T = unit === 'feet' ? thickness * 0.3048 : thickness;
    
    const volumePerWallCum = L * H * T;
    const totalVolumeCum = volumePerWallCum * wallNos;
    ...
    // Vertical Steel
    const heightMM = H * 1000;
    const verticalBarsNos = Math.floor(heightMM / verticalBarSpacing) + 1;
    const verticalBarLengthM = L;
    const verticalTotalLengthM = verticalBarLengthM * verticalBarsNos * wallNos;
    const verticalSteelKg = verticalTotalLengthM * getWeightPerMeter(verticalBarDia);
    
    // Horizontal Steel
    const lengthMM = L * 1000;
    const horizontalBarsNos = Math.floor(lengthMM / horizontalBarSpacing) + 1;
    const horizontalBarLengthM = H;
    const horizontalTotalLengthM = horizontalBarLengthM * horizontalBarsNos * wallNos;
    const horizontalSteelKg = horizontalTotalLengthM * getWeightPerMeter(horizontalBarDia);
    
    // Toe/Heel Steel
    const toeBarsNos = Math.floor(lengthMM / toeBarSpacing) + 1;
    const toeBarLengthM = 0.6;
    const toeTotalLengthM = toeBarLengthM * toeBarsNos * wallNos;
    const toeSteelKg = toeTotalLengthM * getWeightPerMeter(toeBarDia);
    
    const totalSteelKg = (verticalSteelKg + horizontalSteelKg + toeSteelKg) * (1 + wastage/100);
    ...
    const materialTotal = cementCost + sandCost + agg20Cost + agg12Cost + waterCost + steelCost + bindingWireCost + coverBlockCost;
    ...
    const steelBreakdown = [
      ...
      { item: `${toeBarDia}mm Toe/Heel Bars @ ${toeBarSpacing}mm`, quantity: toeSteelKg }
    ];
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Retaining_Wall');
    XLSX.writeFile(wb, `Retaining_Wall_${new Date().toISOString().split('T')[0]}.xlsx`);
  };

  const handleWhatsApp = () => {
    if (!results) return;
    const message = `🧱 RETAINING WALL CALCULATION\n\nLength: ${length} ${unit}\nHeight: ${height} ${unit}\nConcrete: ${results.concrete.volumeCft} CFT\nCement: ${results.concrete.cement} bags\nSteel: ${results.steel.total} kg\nTotal Cost: ₹${results.costs.grandTotal}`;
  ...
  return React.createElement('div', { style: styles.container },
    React.createElement('div', { style: styles.header },
      ...
      React.createElement('h1', { style: styles.headerTitle }, '🧱 Retaining Wall Calculator')
    ),
    ...
    React.createElement('div', { style: styles.row6 },
      ...
      React.createElement('div', null, React.createElement('label', { style: styles.label }, 'Thickness'), React.createElement('input', { type: 'number', value: thickness, onChange: (e) => setThickness(parseFloat(e.target.value)), style: styles.input })),
    ...
    React.createElement('div', { style: styles.row3 },
      ...
      // Toe/Heel Box
      React.createElement('div', { style: styles.subSection },
        React.createElement('div', { style: styles.subTitle }, '🦶 Toe/Heel Bars'),
        React.createElement('div', { style: styles.inputGroup }, React.createElement('label', { style: styles.label }, 'Dia (mm)'), React.createElement('input', { type: 'number', value: toeBarDia, onChange: (e) => setToeBarDia(parseFloat(e.target.value)), style: styles.input })),
        React.createElement('div', { style: styles.inputGroup }, React.createElement('label', { style: styles.label }, 'Spacing (mm)'), React.createElement('input', { type: 'number', value: toeBarSpacing, onChange: (e) => setToeBarSpacing(parseFloat(e.target.value)), style: styles.input }))
      )
    ),
