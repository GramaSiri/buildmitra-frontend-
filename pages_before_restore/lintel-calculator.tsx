...
const styles = {
  ...
  header: { backgroundColor: '#cd853f', padding: '12px', borderRadius: '8px', marginBottom: '15px', color: 'white', display: 'flex', alignItems: 'center', gap: '10px' },
  ...
  sectionTitle: { backgroundColor: '#e8f4f8', color: '#cd853f', padding: '8px', borderRadius: '6px', marginBottom: '12px', fontSize: '14px', fontWeight: 'bold', textAlign: 'center', border: '1px solid #cce5ed' },
  ...
  th: { backgroundColor: '#cd853f', color: 'white', padding: '8px', textAlign: 'left' },
...
export default function LintelPage() {
  ...
  // Lintel Details
  const [lintelNos, setLintelNos] = useState(1);
  const [openingWidth, setOpeningWidth] = useState(4);
  const [bearingLength, setBearingLength] = useState(0.5);
  ...
  const [mainBarNos, setMainBarNos] = useState(2);
  ...
  const labourRate = rates?.labour_concrete || 1000;

  const calculateResults = () => {
    // Lintel Length
    const totalLength = openingWidth + (2 * bearingLength);
    
    // Concrete Volume
    const volumeCum = totalLength * width * depth * lintelNos;
    ...
    // Bottom Bars Steel
    const bottomBarLength = totalLength + 0.3;
    const bottomTotalLengthM = bottomBarLength * mainBarNos * lintelNos;
    const bottomSteelWeight = bottomTotalLengthM * getWeightPerMeter(mainBarDia);
    
    // Top Bars Steel
    const topTotalLengthM = bottomBarLength * topBarNos * lintelNos;
    const topSteelWeight = topTotalLengthM * getWeightPerMeter(topBarDia);
    
    // Stirrups
    const stirrupPerimeterM = 2 * (width + depth) - (8 * cover / 1000) + (24 * stirrupDia / 1000);
    const stirrupNos = Math.ceil(totalLength / (stirrupSpacing / 1000)) + 1;
    const totalStirrupNos = stirrupNos * lintelNos;
    ...
    const coverBlocks = volumeCum * 15;
    ...
    const steelBreakdown = [
      { item: `${mainBarDia}mm Bottom Bars (${mainBarNos} nos)`, quantity: bottomSteelWeight },
    ...
    return {
      lintel: { length: formatNumber(totalLength), width: width, depth: depth, nos: lintelNos },
  ...
  return React.createElement('div', { style: styles.container },
    React.createElement('div', { style: styles.header },
      ...
      React.createElement('h1', { style: styles.headerTitle }, '🚪 Lintel Calculator')
    ),
    ...
    React.createElement('div', { style: styles.sectionTitle }, '📐 Lintel Dimensions'),
    React.createElement('div', { style: styles.row6 },
      React.createElement('div', null, React.createElement('label', { style: styles.label }, 'Nos'), React.createElement('input', { type: 'number', value: lintelNos, onChange: (e) => setLintelNos(parseFloat(e.target.value)), style: styles.input })),
      React.createElement('div', null, React.createElement('label', { style: styles.label }, 'Opening Width (ft)'), React.createElement('input', { type: 'number', value: openingWidth, onChange: (e) => setOpeningWidth(parseFloat(e.target.value)), style: styles.input })),
      React.createElement('div', null, React.createElement('label', { style: styles.label }, 'Bearing (ft)'), React.createElement('input', { type: 'number', value: bearingLength, onChange: (e) => setBearingLength(parseFloat(e.target.value)), style: styles.input })),
    ...
    React.createElement('div', { style: styles.row4 },
      React.createElement('div', null, React.createElement('label', { style: styles.label }, 'Bottom Bar Dia (mm)'), React.createElement('input', { type: 'number', value: mainBarDia, onChange: (e) => setMainBarDia(parseFloat(e.target.value)), style: styles.input })),
      React.createElement('div', null, React.createElement('label', { style: styles.label }, 'Bottom Bar Nos'), React.createElement('input', { type: 'number', value: mainBarNos, onChange: (e) => setMainBarNos(parseFloat(e.target.value)), style: styles.input })),
      React.createElement('div', null, React.createElement('label', { style: styles.label }, 'Top Bar Dia (mm)'), React.createElement('input', { type: 'number', value: topBarDia, onChange: (e) => setTopBarDia(parseFloat(e.target.value)), style: styles.input })),
      React.createElement('div', null, React.createElement('label', { style: styles.label }, 'Top Bar Nos'), React.createElement('input', { type: 'number', value: topBarNos, onChange: (e) => setTopBarNos(parseFloat(e.target.value)), style: styles.input })),
