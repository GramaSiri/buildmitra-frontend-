...
const router = useRouter();
  ...
  const [topBarNos, setTopBarNos] = useState(4);
  
  // Bottom Bars
  const [bottomBarDia, setBottomBarDia] = useState(12);
  const [bottomBarNos, setBottomBarNos] = useState(4);
  ...
  const calculateResults = () => {
    ...
    const volumePerFootingCum = L * W * D;
    const totalVolumeCum = volumePerFootingCum * footingNos;
    ...
    const barLengthM = L;
    const topTotalLengthM = barLengthM * topBarNos * footingNos;
    const topSteelWeight = topTotalLengthM * getWeightPerMeter(topBarDia);
    
    const bottomTotalLengthM = barLengthM * bottomBarNos * footingNos;
    ...
    const stirrupNosPerFooting = Math.ceil(L / (stirrupSpacing / 1000)) + 1;
    const totalStirrupNos = stirrupNosPerFooting * footingNos;
    ...
    const totalSteelKg = topSteelWeight + bottomSteelWeight + stirrupWeight;
    ...
    const labourPerCum = rates?.labour_concrete || 800;
    const labourCost = totalVolumeCum * labourPerCum;
    
    const cementCost = cementBags * (rates?.cement || 385);
    ...
    const steelCost = steelWithWastage * (rates?.steel || 71);
    ...
    const labourBreakdown = {
      shuttering: totalVolumeCum * 320,
      barBending: totalVolumeCum * 240,
      concreting: totalVolumeCum * 160,
      profit: totalVolumeCum * 80,
      total: labourCost
    };
    ...
    return {
      ...
      steel: { top: formatNumber(topSteelWeight), bottom: formatNumber(bottomSteelWeight), stirrups: formatNumber(stirrupWeight), total: formatNumber(steelWithWastage) },
      steelBreakdown,
      ...
      rates: { cement: rates?.cement || 385, sand: rates?.sand || 55, steel: rates?.steel || 71, labourPerCum: labourPerCum }
    };
  };
  ...
  return React.createElement('div', { style: styles.container },
    ...
    React.createElement('div', { style: styles.rateInfo },
      React.createElement('span', null, `💰 Material Rates: Cement ₹${rates?.cement || 385}/bag | Steel ₹${rates?.steel || 71}/kg`),
      React.createElement('div', null, React.createElement('small', null, `👷 Labour: ₹${rates?.labour_concrete || 800}/CUM`))
    ),
    
    React.createElement('div', { style: styles.sectionTitle }, '📐 Footing Dimensions'),
    React.createElement('div', { style: styles.row6 },
      React.createElement('div', null, React.createElement('label', { style: styles.label }, 'Nos'), React.createElement('input', { type: 'number', value: footingNos, onChange: (e) => setFootingNos(parseFloat(e.target.value)), style: styles.input })),
    ...
    React.createElement('div', { style: styles.buttonRow },
      ...
      generated && results && React.createElement(React.Fragment, null,
        React.createElement('button', { onClick: () => checkAndRun('calculator_export', 'footing-calculator.backup-before-fix', handleExportExcel), style: styles.buttonExport }, '📊 Excel'),
        React.createElement('button', { onClick: () => checkAndRun('calculator_export', 'footing-calculator.backup-before-fix', handleWhatsApp), style: styles.buttonWhatsapp }, '💬 Share')
      )
    ),
