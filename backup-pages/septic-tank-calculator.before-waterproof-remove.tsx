...
const router = useRouter();
  ...
  const waterproof = getMasterRate(["waterproofing"], 0, ["bm_service_rates", "bm_material_rates"]);
  ...
  const calc = () => {
    ...
    const waterproofArea = 2 * (innerL + innerW) * H + innerL * innerW;
    ...
    rows.push(
      ...
      ["Waterproofing - Internal Floor + Walls", waterproofArea, "sqft", waterproofArea * waterproof.rate],
  ...
  const rateMsg = rateStatusMessage({ cement, sand, agg20, agg12, steel, waterproof, labour });
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.card}>
        💰 Admin Rates: Cement ₹{cement.rate}/bag | Steel ₹{steel.rate}/kg | PVC ₹{pvc.rate}/set | Waterproofing ₹{waterproof.rate}/sqft
      ...
      <div style={styles.card}>
        ...
        <p style={{ fontSize: 12 }}>Defaults: 1 ft wall, 150mm base slab, 150mm cover slab, 12mm + 10mm steel @ 100mm, PVC inlet/outlet/vent set, waterproofing included.</p>
      </div>

      <div style={{ textAlign: "center" }}>
        <button style={styles.btn} onClick={calc}>🔨 Generate</button>
        {result && <button style={{ ...styles.btn, background: "#28a745" }} onClick={() => checkAndRun('calculator_export', 'septic-tank-calculator.before-waterproof-remove', exportExcel)}>📊 Excel</button>}
