...
const styles = {
  ...
  header: { backgroundColor: '#0288d1', padding: '12px', borderRadius: '8px', marginBottom: '15px', color: 'white', display: 'flex', alignItems: 'center', gap: '10px' },
  ...
  sectionTitle: { backgroundColor: '#e8f4f8', color: '#0288d1', padding: '8px', borderRadius: '6px', marginBottom: '12px', fontSize: '14px', fontWeight: 'bold', textAlign: 'center', border: '1px solid #cce5ed' },
  ...
  th: { backgroundColor: '#0288d1', color: 'white', padding: '8px', textAlign: 'left' },
...
const router = useRouter();
  ...
  const calculateResults = () => {
    ...
    // Water Capacity
    const waterCapacityCum = (L - 2*t) * (W - 2*t) * H;
    const waterCapacityLtr = waterCapacityCum * 1000;
    ...
    const verticalBarsNos = Math.floor(H / (verticalBarSpacing / 1000)) + 1;
    ...
    const horizontalBarsNos = horizontalBarsNosPerLevel * (Math.floor(H / 1) + 1);
    ...
    // Opening Area
    const openingAreaSqft = (openingLength * openingWidth) * openingNos;
    
    // FRP Cover
    const frpCost = openingAreaSqft * frpRate;
    
    // Waterproofing Area (External)
    const waterproofingArea = (2 * (L + W) * H * 10.764 + (L * W * 10.764)) * tankNos;
    const waterproofingCost = waterproofingArea * waterproofingRate;
    
    // Food Grade Paint Area (Internal)
    const paintArea = (2 * (L + W) * H * 10.764 + (L * W * 10.764)) * tankNos;
    ...
    const materialTotal = cementCost + sandCost + agg20Cost + agg12Cost + waterCost + steelCost + bindingWireCost + coverBlockCost + frpCost + waterproofingCost + paintCost;
    ...
    return {
      ...
      capacity: { cum: formatNumber(waterCapacityCum), liters: formatNumber(waterCapacityLtr) },
      area: { total: formatNumber(totalAreaSqft), opening: formatNumber(openingAreaSqft), waterproofing: formatNumber(waterproofingArea), paint: formatNumber(paintArea) },
      ...
      frp: { area: formatNumber(openingAreaSqft), cost: formatNumber(frpCost) },
      waterproofing: { area: formatNumber(waterproofingArea), cost: formatNumber(waterproofingCost) },
      paint: { area: formatNumber(paintArea), cost: formatNumber(paintCost) },
      costs: { cement: formatNumber(cementCost), sand: formatNumber(sandCost), agg20: formatNumber(agg20Cost), agg12: formatNumber(agg12Cost), water: formatNumber(waterCost), steel: formatNumber(steelCost), bindingWire: formatNumber(bindingWireCost), coverBlock: formatNumber(coverBlockCost), frp: formatNumber(frpCost), waterproofing: formatNumber(waterproofingCost), paint: formatNumber(paintCost), materialTotal: formatNumber(materialTotal), labour: formatNumber(labourCost), grandTotal: formatNumber(grandTotal) },
      labourBreakdown: {
  ...
  const handleExportExcel = () => {
    ...
    const data = [
      ...
      { Item: 'Water Capacity', Quantity: results.capacity.liters, Unit: 'Liters', Cost: '-' },
      ...
      { Item: 'FRP Cover (2x2 ft)', Quantity: results.frp.area, Unit: 'sqft', Cost: `₹${results.costs.frp}` },
      { Item: 'Waterproofing', Quantity: results.waterproofing.area, Unit: 'sqft', Cost: `₹${results.costs.waterproofing}` },
      { Item: 'Food Grade Paint', Quantity: results.paint.area, Unit: 'sqft', Cost: `₹${results.costs.paint}` },
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Water_Tank');
    XLSX.writeFile(wb, `Water_Tank_${new Date().toISOString().split('T')[0]}.xlsx`);
  };

  const handleWhatsApp = () => {
    if (!results) return;
    const message = `💧 WATER TANK CALCULATION\n\nSize: ${length} x ${width} x ${height} ${unit}\nCapacity: ${results.capacity.liters} Liters\nConcrete: ${results.concrete.volumeCft} CFT\nCement: ${results.concrete.cement} bags\nSteel: ${results.steel.total} kg\nFRP Cover: ₹${results.costs.frp}\nWaterproofing: ₹${results.costs.waterproofing}\nFood Grade Paint: ₹${results.costs.paint}\nTotal Cost: ₹${results.costs.grandTotal}`;
  ...
  return React.createElement('div', { style: styles.container },
    React.createElement('div', { style: styles.header },
      ...
      React.createElement('h1', { style: styles.headerTitle }, '💧 Water Tank Calculator')
    ),
    ...
    React.createElement('div', { style: styles.row6 },
      ...
      React.createElement('div', null, React.createElement('label', { style: styles.label }, 'Height'), React.createElement('input', { type: 'number', value: height, onChange: (e) => setHeight(parseFloat(e.target.value)), style: styles.input })),
    ...
    React.createElement('div', { style: styles.buttonRow },
      ...
      generated && results && React.createElement(React.Fragment, null,
        React.createElement('button', { onClick: () => checkAndRun('calculator_export', 'water-tank-calculator.backup-before-simple', handleExportExcel), style: styles.buttonExport }, '📊 Excel'),
        React.createElement('button', { onClick: () => checkAndRun('calculator_export', 'water-tank-calculator.backup-before-simple', handleWhatsApp), style: styles.buttonWhatsapp }, '💬 Share')
      )
    ),
      ...
      React.createElement('div', { style: styles.tableContainer },
        React.createElement('table', { style: styles.table },
          ...
          React.createElement('tbody', null,
            ...
            React.createElement('tr', { style: styles.evenRow }, React.createElement('td', { style: styles.td }, 'Water Capacity'), React.createElement('td', { style: styles.td }, results.capacity.liters), React.createElement('td', { style: styles.td }, 'Liters'), React.createElement('td', { style: styles.td }, '-')),
            React.createElement('tr', null, React.createElement('td', { style: styles.td }, 'Total Area'), React.createElement('td', { style: styles.td }, results.area.total), React.createElement('td', { style: styles.td }, 'sqft'), React.createElement('td', { style: styles.td }, '-')),
            ...
            React.createElement('tr', { style: styles.evenRow }, React.createElement('td', { style: styles.td }, 'FRP Cover (2x2 ft)'), React.createElement('td', { style: styles.td }, results.frp.area), React.createElement('td', { style: styles.td }, 'sqft'), React.createElement('td', { style: styles.td }, `₹${results.costs.frp}`)),
            ...
            React.createElement('tr', { style: styles.evenRow }, React.createElement('td', { style: styles.td }, 'Food Grade Paint'), React.createElement('td', { style: styles.td }, results.paint.area), React.createElement('td', { style: styles.td }, 'sqft'), React.createElement('td', { style: styles.td }, `₹${results.costs.paint}`)),
