...
const blockTypes = {
  'Clay Brick': { length: 9, height: 3, width: 4.5, rateName: 'brick' },
  'Clay Brick (Modular)': { length: 8, height: 3, width: 4, rateName: 'brick' },
  'Concrete Block (4")': { length: 16, height: 8, width: 4, rateName: 'block' },
  'Concrete Block (6")': { length: 16, height: 8, width: 6, rateName: 'block' },
  'Concrete Block (8")': { length: 16, height: 8, width: 8, rateName: 'block' },
  'AAC Block (4")': { length: 24, height: 8, width: 4, rateName: 'aac' },
  'AAC Block (6")': { length: 24, height: 8, width: 6, rateName: 'aac' },
  'AAC Block (8")': { length: 24, height: 8, width: 8, rateName: 'aac' },
  'Interlock Block': { length: 12, height: 6, width: 4, rateName: 'interlock' },
  'Hollow Block (4")': { length: 16, height: 8, width: 4, rateName: 'hollow' },
  'Hollow Block (6")': { length: 16, height: 8, width: 6, rateName: 'hollow' },
  'Solid Block': { length: 12, height: 6, width: 4, rateName: 'solid' }
};
...
const router = useRouter();
  ...
  const [mortarThicknessMm, setMortarThicknessMm] = useState(10);
  ...
  const calculateResults = () => {
    const wallAreaSqft = Number(length || 0) * Number(height || 0) * Number(wallNos || 0);
    const totalOpeningArea = openings.reduce((sum, opening) => sum + Number(opening.area || 0), 0);
    ...
    const moduleHeightIn = blockInfo.height + mortarInch;
    const moduleFaceAreaSqft = (moduleLengthIn * moduleHeightIn) / 144;

    const blocksWithoutWastage = moduleFaceAreaSqft > 0 ? netArea / moduleFaceAreaSqft : 0;
    const totalBlocks = blocksWithoutWastage * (1 + Number(wastage || 0) / 100);

    const wallVolumeCft = netArea * (blockInfo.width / 12);
    const actualBlockVolumeCft = blocksWithoutWastage * ((blockInfo.length * blockInfo.height * blockInfo.width) / 1728);
    const mortarVolumeCft = Math.max(wallVolumeCft - actualBlockVolumeCft, 0);
    const dryMortarVolumeCft = mortarVolumeCft * 1.33;
    const mortarVolumeCum = mortarVolumeCft / 35.315;

    const [cementPart, sandPart] = mortarGrade.split(':').map(Number);
    const totalPart = cementPart + sandPart;

    const cementCft = dryMortarVolumeCft * (cementPart / totalPart);
    const cementBags = cementCft / 1.25;
    const sandCft = dryMortarVolumeCft * (sandPart / totalPart);
    ...
    return {
      ...
      blocks: {
        type: blockType,
        size: `${blockInfo.length}"x${blockInfo.height}"x${blockInfo.width}"`,
        quantity: formatNumber(totalBlocks),
        perSqft: formatNumber(moduleFaceAreaSqft > 0 ? 1 / moduleFaceAreaSqft : 0),
        rate: blockRate
      },
      mortar: {
        thicknessMm: mortarThicknessMm,
        volumeCft: formatNumber(mortarVolumeCft),
        dryVolumeCft: formatNumber(dryMortarVolumeCft),
        volumeCum: formatNumber(mortarVolumeCum)
      },
      materials: { cement: formatNumber(cementBags), sand: formatNumber(sandCft), water: formatNumber(waterLtr) },
      costs: {
        blocks: formatNumber(blockCost),
        cement: formatNumber(cementCost),
        sand: formatNumber(sandCost),
        water: formatNumber(waterCost),
  ...
  const handleExportExcel = () => {
    ...
    const data = [
      ...
      { Item: 'Mortar Thickness', Quantity: results.mortar.thicknessMm, Unit: 'mm', Cost: '-' },
      { Item: 'Mortar Wet Volume', Quantity: results.mortar.volumeCft, Unit: 'CFT', Cost: '-' },
      { Item: 'Mortar Dry Volume', Quantity: results.mortar.dryVolumeCft, Unit: 'CFT', Cost: '-' },
  ...
  return React.createElement('div', { style: styles.container },
    ...
    React.createElement('div', { style: styles.rateInfo },
      ...
      React.createElement('div', null, React.createElement('small', null, `👷 Labour: ₹${labourRate}/sqft`))
    ),

    React.createElement('div', { style: styles.sectionTitle }, '📐 Wall Dimensions'),
    React.createElement('div', { style: styles.row6 },
      React.createElement('div', null, React.createElement('label', { style: styles.label }, 'Nos'), React.createElement('input', { type: 'number', value: wallNos, onChange: (e) => setWallNos(parseFloat(e.target.value) || 0), style: styles.input })),
      React.createElement('div', null, React.createElement('label', { style: styles.label }, 'Length (ft)'), React.createElement('input', { type: 'number', value: length, onChange: (e) => setLength(parseFloat(e.target.value) || 0), style: styles.input })),
      React.createElement('div', null, React.createElement('label', { style: styles.label }, 'Height (ft)'), React.createElement('input', { type: 'number', value: height, onChange: (e) => setHeight(parseFloat(e.target.value) || 0), style: styles.input })),
      ...
      React.createElement('div', null, React.createElement('label', { style: styles.label }, 'Mortar Thick. (mm)'), React.createElement('input', { type: 'number', value: mortarThicknessMm, onChange: (e) => setMortarThicknessMm(parseFloat(e.target.value) || 0), style: styles.input })),
    ...
    React.createElement('div', { style: styles.row6 },
      ...
      React.createElement('div', null, React.createElement('label', { style: styles.label }, 'Length (ft)'), React.createElement('input', { type: 'number', value: newOpening.length, onChange: (e) => setNewOpening({ ...newOpening, length: parseFloat(e.target.value) || 0 }), style: styles.input })),
      React.createElement('div', null, React.createElement('label', { style: styles.label }, 'Width (ft)'), React.createElement('input', { type: 'number', value: newOpening.width, onChange: (e) => setNewOpening({ ...newOpening, width: parseFloat(e.target.value) || 0 }), style: styles.input })),
      React.createElement('div', null, React.createElement('label', { style: styles.label }, 'Nos'), React.createElement('input', { type: 'number', value: newOpening.nos, onChange: (e) => setNewOpening({ ...newOpening, nos: parseFloat(e.target.value) || 0 }), style: styles.input })),
    ...
    React.createElement('div', { style: styles.buttonRow },
      ...
      generated && results && React.createElement(React.Fragment, null,
        React.createElement('button', { onClick: () => checkAndRun('calculator_export', 'brick-calculator', handleExportExcel), style: styles.buttonExport }, '📊 Excel'),
        React.createElement('button', { onClick: () => checkAndRun('calculator_export', 'brick-calculator', handleWhatsApp), style: styles.buttonWhatsapp }, '💬 Share')
      )
    ),
      ...
      React.createElement('div', { style: styles.tableContainer },
        React.createElement('table', { style: styles.table },
          ...
          React.createElement('tbody', null,
            ...
            React.createElement('tr', null, React.createElement('td', { style: styles.td }, 'Mortar Thickness'), React.createElement('td', { style: styles.td }, results.mortar.thicknessMm), React.createElement('td', { style: styles.td }, 'mm'), React.createElement('td', { style: styles.td }, '-')),
            React.createElement('tr', { style: styles.evenRow }, React.createElement('td', { style: styles.td }, 'Mortar Wet Volume'), React.createElement('td', { style: styles.td }, results.mortar.volumeCft), React.createElement('td', { style: styles.td }, 'CFT'), React.createElement('td', { style: styles.td }, '-')),
