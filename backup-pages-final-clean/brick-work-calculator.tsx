...
const styles = {
  ...
  row6: { display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '8px', marginBottom: '12px' },
...
const router = useRouter();
  ...
  const sandRate = getMasterRate(["sand"], rates?.sand || 55).rate;
  ...
  const calculateResults = () => {
    ...
    const blocksPerSqft = moduleFaceAreaSqft > 0 ? 1 / moduleFaceAreaSqft : 0;
    ...
    const cementBags = (dryMortarVolumeCft * (cementPart / totalPart)) / 1.25;
    ...
    return {
      ...
      mortar: { thicknessMm: mortarThicknessMm, volumeCft: formatNumber(mortarVolumeCft), dryVolumeCft: formatNumber(dryMortarVolumeCft), volumeCum: formatNumber(mortarVolumeCum) },
  ...
  return React.createElement('div', { style: styles.container },
    ...
    React.createElement('div', { style: styles.rateInfo },
      React.createElement('span', null, `💰 Admin Master Rates: ${blockType} ₹${blockRate}/nos | Cement ₹${cementRate}/bag | Sand ₹${sandRate}/CFT`),
    ...
    React.createElement('div', { style: styles.buttonRow },
      ...
      generated && results && React.createElement(React.Fragment, null,
        React.createElement('button', { onClick: () => checkAndRun('calculator_export', 'brick-work-calculator', handleExportExcel), style: styles.buttonExport }, '📊 Excel'),
        React.createElement('button', { onClick: () => checkAndRun('calculator_export', 'brick-work-calculator', handleWhatsApp), style: styles.buttonWhatsapp }, '💬 Share')
      )
    ),
