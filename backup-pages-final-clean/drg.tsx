...
import { hasPaidUnlock, markPaidUnlock } from "../utils/accessControl";
...
function snap(n:number){return Number(n.toFixed(2))}
function roomIntersects(a:RoomRect,b:RoomRect,buffer=.05){
  return Math.min(a.x+a.w,b.x+b.w)-Math.max(a.x,b.x)>buffer&&Math.min(a.y+a.h,b.y+b.h)-Math.max(a.y,b.y)>buffer;
}
function clampRoomInside(room:RoomRect,footprint:{width:number;length:number}){
  room.x=snap(clamp(room.x,0,Math.max(0,footprint.width-room.w)));
  room.y=snap(clamp(room.y,0,Math.max(0,footprint.length-room.h)));
  room.w=snap(Math.min(room.w,footprint.width-room.x));
  room.h=snap(Math.min(room.h,footprint.length-room.y));
  return room;
}
function addArchitecturalFillerRooms(rooms:RoomRect[],input:Inputs,footprint:{width:number;length:number},level:number){
  const xs=Array.from(new Set([0,footprint.width,...rooms.flatMap(r=>[r.x,r.x+r.w]).map(v=>snap(v))])).sort((a,b)=>a-b);
  const ys=Array.from(new Set([0,footprint.length,...rooms.flatMap(r=>[r.y,r.y+r.h]).map(v=>snap(v))])).sort((a,b)=>a-b);
  const fillers:RoomRect[]=[];
  const maxFillers=input.plotWidth*input.plotLength>1800?8:5;
  for(let yi=0;yi<ys.length-1;yi++)for(let xi=0;xi<xs.length-1;xi++){
    if(fillers.length>=maxFillers)break;
    const x=xs[xi],y=ys[yi],w=snap(xs[xi+1]-x),h=snap(ys[yi+1]-y),area=w*h;
    if(w<2.2||h<2.2||area<12)continue;
    const probe=makeRoom("probe",level,"Unit 1","probe","core",x+.04,y+.04,Math.max(.01,w-.08),Math.max(.01,h-.08),"");
    if(rooms.some(r=>roomIntersects(r,probe,.02))||fillers.some(r=>roomIntersects(r,probe,.02)))continue;
    const nearKitchen=rooms.some(r=>r.kind==="kitchen"&&Math.abs((r.x+r.w)-(x))<.2||r.kind==="kitchen"&&Math.abs((x+w)-r.x)<.2);
    const nearMaster=rooms.some(r=>r.name.includes("Master")&&Math.abs((r.x+r.w)-x)<.2);
    const front=y<footprint.length*.18||y+h>footprint.length*.82;
    let name="Store",kind:RoomRect["kind"]="utility",notes="Blank-space resolver converted leftover area into usable store/service pocket";
    if(area<18){name="Ventilation Shaft";kind="core";notes="Small unusable pocket converted into ventilation/service shaft";}
    else if(nearKitchen){name="Wash / Utility";notes="Leftover pocket attached to kitchen as wash/utility area";}
    else if(nearMaster){name="Dress / Linen";notes="Leftover pocket near bedroom converted into dress/linen area";}
    else if(front){name=input.facing==="North"||input.facing==="South"?"Sitout / OTS":"Open-to-Sky Pocket";notes="Front/rear leftover pocket converted into sitout/open-to-sky space";}
    const filler=makeRoom(`f${level}-filler-${fillers.length+1}`,level,"Unit 1",name,kind,x,y,w,h,notes);
    if(kind==="core")addWindow(filler,w>h?"north":"east",.5,true);else addDoor(filler,y<footprint.length/2?"south":"north",.5);
    fillers.push(filler);
  }
  return [...rooms,...fillers];
}
function finalizeArchitectLayout(rooms:RoomRect[],input:Inputs,footprint:{width:number;length:number},level:number){
  const priority=(r:RoomRect)=>{
    if(r.kind==="bed")return 1;
    if(r.kind==="toilet")return 2;
    if(r.kind==="stair"||r.kind==="lift")return 3;
    if(r.kind==="kitchen")return 4;
    if(r.kind==="living")return 5;
    if(r.kind==="dining")return 6;
    if(r.kind==="foyer"||r.kind==="pooja")return 7;
    if(r.kind==="balcony")return 8;
    if(r.kind==="utility")return 9;
    if(r.kind==="passage")return 10;
    return 11;
  };
  const cleaned=rooms.map(r=>clampRoomInside(r,footprint)).filter(r=>r.w>=2&&r.h>=2).sort((a,b)=>priority(a)-priority(b));
  const nonOverlapping:RoomRect[]=[];
  cleaned.forEach(room=>{
    if(!nonOverlapping.some(existing=>roomIntersects(existing,room,.18)))nonOverlapping.push(room);
  });
  return attachDoorsWindows(addArchitecturalFillerRooms(nonOverlapping,input,footprint,level),input,footprint);
}
...
function createSingleUnitRectangles(input:Inputs,footprint:{width:number;length:number},level:number){
  ...
  const livingW=clampFit(W*.48,14,20),livingH=clampFit(L*.34,10,13);
  const bedW=clampFit(W*.31,10,12.5),bedH=clampFit(L*.40,10,13);
  ...
  const stairW=clampFit(W*.17,6,7.5),stairH=clampFit(L*.40,12,14),stairRunH=input.lift?Math.max(9,stairH-4):stairH,liftH=input.lift?Math.max(4,stairH-stairRunH):0;
  const coreX=Math.max(0,W-stairW),coreY=Math.max(0,L-stairH);
  if(level===1){
    const balconyH=input.balcony?clampFit(L*.09,3,4):0;
    const frontY=input.facing==="South"?L-balconyH-livingH:balconyH;
    const rearY=input.facing==="South"?0:L-bedH;
    const entrySide:RoomRect["doors"][number]["side"]=input.facing==="East"?"east":input.facing==="West"?"west":input.facing==="South"?"south":"north";
    const foyerW=clampFit(W*.16,4,6),foyerH=clampFit(L*.16,5,7);
    const foyerX=input.facing==="East"?W-foyerW:input.facing==="West"?0:Math.max(0,(W-foyerW)/2);
    const foyerY=input.facing==="South"?L-foyerH-balconyH:input.facing==="North"?balconyH:frontY;
    const livingX=input.facing==="West"?foyerW:input.facing==="East"?bedW:Math.max(0,(W-livingW)/2);
    const livingY=input.facing==="North"?foyerY+foyerH:input.facing==="South"?Math.max(0,foyerY-livingH):frontY;
    const livingW2=input.facing==="East"||input.facing==="West"?Math.max(12,W-bedW-foyerW):livingW;
    const livingH2=input.facing==="North"||input.facing==="South"?Math.max(9,livingH-foyerH*.35):livingH;
    const bed2X=0,bed2Y=0;
    const masterW=Math.max(12,Math.min(14,W*.40)),masterBalconyH=input.balcony?Math.min(3.5,Math.max(3,bedH-9.5)):0,masterBedH=bedH-masterBalconyH;
    const masterY=L-bedH;
    const serviceTop=Math.max(livingY+livingH2,bedH),serviceBand=Math.max(8,masterY-serviceTop);
    const diningX=bedW,diningY=serviceTop,diningW=Math.max(8,Math.min(13,W-bedW-kitchenW)),diningH=Math.max(5,Math.min(7,serviceBand*.58));
    const kitchenX=Math.min(W-kitchenW,bedW+diningW),kitchenY=serviceTop,kitchenH2=Math.min(kitchenH,Math.max(8,coreY-serviceTop));
    const passageX=bedW,passageY=diningY+diningH,rawPassageW=Math.max(4,Math.min(W-bedW-stairW,kitchenX-bedW)),passageH=Math.max(3.5,Math.min(5,masterY-passageY));
    const commonToiletX=Math.min(W-stairW-toiletW,Math.max(masterW+toiletW+.3,kitchenX-toiletW));
    const passageW=Math.max(3.5,Math.min(rawPassageW,commonToiletX-passageX));
    const poojaW=4.5,poojaH=5;
    const utilityX=kitchenX,utilityY=Math.min(L-3.5,kitchenY+kitchenH2),utilityH=Math.max(0,Math.min(3.5,L-(kitchenY+kitchenH2)));

    rooms.push(addDoor(addWindow(makeRoom(`f${level}-foyer`,level,"Unit 1","Entry Foyer","foyer",foyerX,foyerY,foyerW,foyerH,"Main entry connects road side to living without crossing private rooms"),input.facing==="South"?"south":"north"),entrySide,.5));
    rooms.push(addDoor(addWindow(makeRoom(`f${level}-bed2`,level,"Unit 1","Bedroom 2","bed",bed2X,bed2Y,bedW,bedH,"NW/West bedroom connected from passage/living"),"west"),"east",.55));
    rooms.push(addDoor(addWindow(makeRoom(`f${level}-living`,level,"Unit 1","Living","living",livingX,livingY,livingW2,livingH2,"Living preferred North/East/NE; connects foyer, dining and passage"),input.facing==="South"?"south":"north"),input.facing==="East"?"east":input.facing==="West"?"west":input.facing==="South"?"south":"north",.45));
    if(input.balcony){
      const livingBalY=input.facing==="South"?L-balconyH:0;
      rooms.push(addDoor(addWindow(makeRoom(`f${level}-balcony`,level,"Unit 1","Living Balcony","balcony",livingX,livingBalY,livingW2,balconyH,"Balcony physically connected to living room"),input.facing==="South"?"south":"north"),input.facing==="South"?"north":"south",.5));
    }
    if(input.poojaRoom)rooms.push(addDoor(addWindow(makeRoom(`f${level}-pooja`,level,"Unit 1","Pooja","pooja",Math.max(0,W-poojaW),input.facing==="East"?Math.min(L-poojaH,foyerY+foyerH):0,poojaW,poojaH,"NE pooja pocket near living/foyer"),"north"),"west"));
    rooms.push(addDoor(makeRoom(`f${level}-dining`,level,"Unit 1","Dining","dining",diningX,diningY,diningW,diningH,"Central dining links living to kitchen"),"north"));
    rooms.push(addDoor(makeRoom(`f${level}-passage`,level,"Unit 1","Internal Passage","passage",passageX,passageY,passageW,passageH,"Compact internal passage connects bedrooms, toilets, dining and stair lobby"),"north"));
    rooms.push(addDoor(addWindow(makeRoom(`f${level}-kitchen`,level,"Unit 1","Kitchen","kitchen",kitchenX,kitchenY,kitchenW,kitchenH2,"SE/NW kitchen with external window and dining access"),"east"),"west",.42));
    if(utilityH>=2.5)rooms.push(addDoor(addWindow(makeRoom(`f${level}-utility`,level,"Unit 1","Utility","utility",utilityX,utilityY,kitchenW,utilityH,"Utility attached to kitchen to use leftover service strip"),"south"),"north"));
    ...
    if(input.toilets>=2)rooms.push(addDoor(addWindow(makeRoom(`f${level}-commonwc`,level,"Unit 1","Common Toilet","toilet",commonToiletX,Math.max(passageY,Math.min(coreY-toiletH,passageY+1)),toiletW,toiletH,"Common toilet opens from internal passage/living, not isolated"),"east",.5,true),"west",.5,true));
    rooms.push(addDoor(makeRoom(`f${level}-stair`,level,"Common","Vaastu Staircase","stair",coreX,coreY,stairW,stairRunH,"Stair placed in South/West service core and connected to passage/lobby"),"west"));
    if(input.lift)rooms.push(addDoor(makeRoom(`f${level}-lift`,level,"Common","Lift","lift",coreX,coreY+stairRunH,Math.min(5.5,stairW),liftH,"Lift adjacent to staircase and opens to internal lobby"),"west"));
  }else{
    ...
    rooms.push(addDoor(makeRoom(`f${level}-stair`,level,"Common","Auto Staircase","stair",coreX,coreY,stairW,stairRunH,"Vertical core aligned floor-to-floor"),"north"));
    if(input.lift)rooms.push(addDoor(makeRoom(`f${level}-lift`,level,"Common","Lift","lift",coreX,coreY+stairRunH,Math.min(5.5,stairW),liftH,"Lift connected to stair lobby"),"north"));
  }
  return finalizeArchitectLayout(rooms,input,footprint,level);
}
...
export default function DrgDraftingEngineV3(){
  ...
  const generate=()=>{
    ...
    setGenerated(normalized);setDrawingCode(code);setExportPaid(typeof window!=="undefined"&&(hasPaidUnlock("drawing_export",code)||localStorage.getItem("buildmitraDrgExportPaid")==="true"||localStorage.getItem(`buildmitraDrawingPaid:${code}`)==="true"));
  ...
  const requirePaid=(action:"PDF"|"Image"|"Excel"|"DXF"|"WhatsApp")=>{
    ...
    if(!exportPaid&&!hasPaidUnlock("drawing_export",drawingCode)){setPaymentModal(true);return;}
    if(action==="WhatsApp"){
  ...
  const markPaid=()=>{
    if(typeof window!=="undefined"){markPaidUnlock("drawing_export",drawingCode||"global");localStorage.setItem("buildmitraDrgExportPaid","true");if(drawingCode)localStorage.setItem(`buildmitraDrawingPaid:${drawingCode}`,"true");}
