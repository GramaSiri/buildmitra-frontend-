...
export default function PlasterCalculator() {
  ...
  const waterRate = getMasterRate(["MAT-WTR-01", "construction water", "water"], 0.05); // Ã¢â€šÂ¹0.05 per Litre
  ...
  const handleShareWhatsApp = () => {
    checkAndRun('calculator_export', 'plaster-calculator', () => {
      ...
      const msg = `*BuildMitra Plaster Calculator Report*%0A` +
        ...
        `----------------------------------------%0A` +
        `Ã¢â‚¬Â¢ *Net Plaster Area*: ${formatNumber(res.netArea)} Sqft%0A` +
        `Ã¢â‚¬Â¢ *Cement (50kg OPC 53)*: ${formatNumber(res.cemBags)} Bags%0A` +
        `Ã¢â‚¬Â¢ *M-Sand / P-Sand*: ${formatNumber(res.sandCft)} CFT%0A` +
...
{/* 1. Header */}
      ...
      <div style={styles.dropdownCard}>
        ...
        >
          ...
          <option value="detailed">Ã°Å¸â€œÂ Detailed Wall-Wise Calculation (Exact Room Dimensions & IS 1200 Deductions)</option>
        </select>
      </div>
        </div>
        ...
        <>
          <div style={styles.stepperCard}>
            <div style={styles.sectionHeader}>
              <span>Ã¢Å¡Â¡ Quick Calculation (Plot Dimensions & BUA Civil Thumb Rule)</span>
            </div>

            <div style={styles.noteBox}>
              Ã°Å¸â€™Â¡ <strong>Civil Engineering Thumb Rule</strong>: Built-Up Area (BUA) = Plot Length x Plot Width x Floors. Plasterable Surface Area (Walls & Ceilings) = BUA x 3.5 Sqft.
            </div>
          ...
          <div style={styles.stepperCard}>
            <div style={styles.sectionHeader}>
              <span>Ã°Å¸â€œÅ  Plaster Calculation Results & Materials BOQ</span>
            </div>
      ...
      {calcMode === 'detailed' && (
        <div style={styles.stepperCard}>
          <div style={styles.sectionHeader}>
            <span>Ã°Å¸â€œÂ Detailed Wall-Wise Plaster Calculation Inputs (IS 1661 & IS 1200 Exact Rules)</span>
          </div>
          ...
          <div style={{ backgroundColor: '#f1f5f9', padding: '12px', borderRadius: '8px', marginBottom: '16px' }}>
            <div style={{ fontSize: '13px', fontWeight: '700', color: '#0f766e', marginBottom: '8px' }}>Ã°Å¸Å¡Âª Openings Deduction (IS 1200 Part 12 Standard Rules)</div>
            <div style={styles.grid4}>
              <div style={styles.fieldGroup}>
          ...
          <div style={{ marginTop: '16px' }}>
            <div style={styles.sectionHeader}>
              <span>Ã°Å¸â€œÅ  Detailed Plaster Calculation Results & Materials BOQ</span>
            </div>
