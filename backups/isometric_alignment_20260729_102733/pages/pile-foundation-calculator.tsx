  ...
  const handleShareWhatsApp = () => {
    checkAndRun('calculator_export', 'pile-foundation-calculator', () => {
      const msg = `*BuildMitra RCC Pile Foundation Report*%0A` +
        ...
        `----------------------------------------%0A` +
        `â€¢ *Piles*: ${inputs.pileNos} Nos (${inputs.diameterFt}' Dia x ${inputs.lengthFt}ft Depth - ${inputs.grade})%0A` +
        `â€¢ *Auger Pit Boring*: ${formatNumber(calcResults.boringLenM)} RMT%0A` +
        `â€¢ *Concrete Volume*: ${formatNumber(calcResults.totalVolCft)} CFT (${formatNumber(calcResults.totalVolCum, 3)} CUM)%0A` +
        (calcResults.hasConcrete ? `â€¢ *Cement*: ${formatNumber(calcResults.cementBags)} Bags%0A` : '') +
        (calcResults.hasSteel ? `â€¢ *Steel Rebar Cage*: ${formatNumber(calcResults.totalSteelKg)} kg (${inputs.mainDia}mm Main + ${inputs.tieDia}mm Ties)%0A` : '') +
        `â€¢ *Material & Boring Total*: ${formatCurrency(calcResults.grandMatCost)}%0A` +
    ...
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        ...
        <h1 style={styles.headerTitle}>
          ðŸ¥§ Pile Foundation Structural & Boring Calculator
      ...
      <MarketRateTrend />
      <EngineeringSpecimen kind="pile" title="Dynamic Pile Specimen" material={inputs.grade} data={{ lengthFt: inputs.lengthFt, diameterMm: inputs.diameterFt * 304.8, grade: inputs.grade, mainDia: inputs.mainDia, mainNos: inputs.mainNos, tieSpacingMm: inputs.tieSpacingMm }} />
      ...
      <div style={styles.dropdownCard}>
        ...
        >
          <option value="both">ðŸ”µ Both Concrete Materials, Boring & Steel Rebar (Complete RCC Pile Foundation)</option>
          <option value="boring_only">ðŸ§± Only Concrete Materials & Auger Pit Boring (No Steel Rebar Cage)</option>
          <option value="cage_only">âš™ï¸ Only Steel Rebar Cage & Circular Helical Ties (Bar Bending Schedule)</option>
        </select>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>ðŸ“ Pile Geometry & Boring Specifications</span>
        </div>

        <div style={styles.noteBox}>
          ðŸ’¡ <strong>IS 2911 Pile Foundation Design Standards</strong>: Computes auger pit boring depth in RMT, tremie concrete volume, main longitudinal bar schedule with $50d$ anchorage, circular helical ties/rings, and heavy duty circular wheel spacers (50mm cover).
        </div>
        ...
        {calcResults.hasSteel && (
          <div style={{ backgroundColor: '#fff5f7', padding: '14px', borderRadius: '8px', marginBottom: '16px', border: '1px solid #fecdd3' }}>
            <div style={{ fontSize: '13px', fontWeight: '700', color: '#800020', marginBottom: '10px' }}>ðŸ”„ Reinforcement Cage & Circular Helical Ties (Bar Bending Schedule)</div>
            <div style={styles.grid4}>
              <div style={styles.fieldGroup}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px' }}>
          <button style={styles.btnReset} onClick={handleReset}>ðŸ”„ Reset Pile Form</button>
        </div>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>ðŸ“Š Pile Foundation Results BOQ ({scopeOption === 'both' ? 'Concrete & Steel' : scopeOption === 'boring_only' ? 'Concrete & Boring' : 'Steel Cage Only'})</span>
        </div>
