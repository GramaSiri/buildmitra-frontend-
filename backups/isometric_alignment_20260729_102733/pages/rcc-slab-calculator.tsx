...
export default function RCCSlabCalculator() {
  ...
  const waterRate = getMasterRate(["MAT-WTR-01", "construction water", "water"], 0.05); // â‚¹0.05 / Ltr
  ...
  const handleShareWhatsApp = () => {
    checkAndRun('calculator_export', 'rcc-slab-calculator', () => {
      ...
      const msg = `*BuildMitra RCC Slab & Beam Calculator Report*%0A` +
        ...
        `----------------------------------------%0A` +
        `â€¢ *Total Concrete Volume*: ${formatNumber(res.totalVolCft)} CFT (${formatNumber(res.totalVolCum)} mÂ³)%0A` +
        `â€¢ *Cement (50kg OPC 53)*: ${formatNumber(res.cemBags)} Bags%0A` +
        `â€¢ *TMT Steel Rebar*: ${formatNumber(res.steelKg)} kg%0A` +
        `â€¢ *M-Sand*: ${formatNumber(res.sandCft)} CFT%0A` +
        `â€¢ *20mm Aggregate*: ${formatNumber(res.agg20Cft)} CFT%0A` +
        `â€¢ *Curing Water*: ${formatNumber(res.waterLtr)} Litres%0A` +
        `â€¢ *Material Total*: ${formatCurrency(res.grandMatCost)}%0A` +
        `â€¢ *Labour Total*: ${formatCurrency(res.grandLabCost)}%0A` +
        `â€¢ *TOTAL ESTIMATED COST*: ${formatCurrency(res.grandTotal)} (${formatCurrency(res.costPerCft)}/CFT)%0A%0A` +
  ...
  return (
    <div style={styles.container}>
      ...
      <MarketRateTrend />
      <EngineeringSpecimen kind="slab" title="Dynamic RCC Slab Specimen" material={calcMode === 'quick' ? quickInputs.concreteGrade : detailedInputs.grade} data={calcMode === 'quick' ? { lengthFt: quickInputs.plotLength, widthFt: quickInputs.plotWidth, depthIn: quickInputs.slabThicknessMm / 25.4, grade: quickInputs.concreteGrade } : { lengthFt: detailedInputs.length, widthFt: detailedInputs.width, depthIn: detailedInputs.thicknessMm / 25.4, grade: detailedInputs.grade, mainDia: detailedInputs.mainDia, distDia: detailedInputs.distDia }} />
      ...
      <div style={styles.dropdownCard}>
        ...
        >
          <option value="quick">âš¡ Quick Calculation (Estimate from Plot Dimensions L x W x Floors BUA)</option>
          <option value="detailed">ðŸ“ Detailed Structural Calculation (Exact Reinforcement & Beam Schedule)</option>
        </select>
      </div>
        ...
        <>
          <div style={styles.stepperCard}>
            <div style={styles.sectionHeader}>
              <span>âš¡ Quick Calculation (Plot Dimensions & Slab Thickness Rule)</span>
            </div>

            <div style={styles.noteBox}>
              ðŸ’¡ <strong>IS 456 Structural Standard</strong>: Concrete Volume = Built-Up Area x Slab Thickness + Monolithic Beam Volume Allowance (~12%). Steel Rebar density = 1.0% volume ratio.
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px' }}>
              <button style={styles.btnReset} onClick={handleResetQuick}>ðŸ”„ Reset Quick Form</button>
            </div>
          </div>
          ...
          <div style={styles.stepperCard}>
            <div style={styles.sectionHeader}>
              <span>ðŸ“Š RCC Slab & Beam Calculation Results & Materials BOQ</span>
            </div>

            {quickCalcResults.unpricedCount > 0 ? (
              <div style={styles.warnBanner}>
                âš ï¸ Partial Estimate: Admin Master Rates unavailable for: {quickCalcResults.unpricedList.join(', ')}.
              </div>
            ) : (
              <div style={{ backgroundColor: '#f0fdf4', border: '1px solid #bbf7d0', color: '#166534', padding: '10px', borderRadius: '8px', marginBottom: '16px', fontSize: '12px', fontWeight: '700' }}>
                âœ“ Complete Estimate: All rates verified against BuildMitra Admin Master Database.
              </div>
            )}
            ...
            <div style={styles.summaryGrid}>
              <div style={{ ...styles.metricCard, ...styles.metricBlue }}>
                ...
                <span style={{ fontSize: '11px', opacity: 0.9 }}>({formatNumber(quickCalcResults.totalVolCum)} mÂ³)</span>
              </div>
      ...
      {calcMode === 'detailed' && (
        <div style={styles.stepperCard}>
          <div style={styles.sectionHeader}>
            <span>ðŸ“ Detailed Structural RCC Calculation Inputs (IS 456 Bar Bending & Beam Schedule)</span>
          </div>
          ...
          <div style={{ backgroundColor: '#f1f5f9', padding: '14px', borderRadius: '8px', marginBottom: '16px' }}>
            <div style={{ fontSize: '13px', fontWeight: '700', color: '#1e3a8a', marginBottom: '10px' }}>ðŸ”„ Slab Reinforcement Details (IS 456 Reinforcement Schedule)</div>
          ...
          <div style={{ backgroundColor: '#eff6ff', padding: '14px', borderRadius: '8px', marginBottom: '16px', border: '1px solid #bfdbfe' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              <div style={{ fontSize: '13px', fontWeight: '700', color: '#1e3a8a' }}>ðŸ“Š Beam Structural Schedule (IS 456 Beam Design)</div>
              <button
              ...
              >
                {detailedInputs.hasBeam ? "âŒ Exclude Beams" : "âœ… Include Beams"}
              </button>
            </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '16px' }}>
            <button style={styles.btnReset} onClick={handleResetDetailed}>ðŸ”„ Reset Detailed Form</button>
          </div>

          {/* DETAILED RESULTS TABLE */}
          <div style={{ marginTop: '16px' }}>
            <div style={styles.sectionHeader}>
              <span>ðŸ“Š Detailed Structural RCC Calculation Results & Materials BOQ</span>
            </div>

            {detailedCalcResults.unpricedCount > 0 ? (
              <div style={styles.warnBanner}>
                âš ï¸ Partial Estimate: Admin Master Rates unavailable for: {detailedCalcResults.unpricedList.join(', ')}.
              </div>
            ) : (
              <div style={{ backgroundColor: '#f0fdf4', border: '1px solid #bbf7d0', color: '#166534', padding: '10px', borderRadius: '8px', marginBottom: '16px', fontSize: '12px', fontWeight: '700' }}>
                âœ“ Complete Estimate: All rates verified against BuildMitra Admin Master Database.
            ...
            <div style={styles.summaryGrid}>
              <div style={{ ...styles.metricCard, ...styles.metricBlue }}>
                ...
                <span style={{ fontSize: '11px', opacity: 0.9 }}>({formatNumber(detailedCalcResults.totalVolCum)} mÂ³)</span>
              </div>
              <div style={{ ...styles.metricCard, ...styles.metricTeal }}>
                <span style={styles.metricTitle}>Cement & Steel</span>
