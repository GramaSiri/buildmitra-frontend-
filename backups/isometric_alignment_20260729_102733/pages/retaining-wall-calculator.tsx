  ...
  const handleShareWhatsApp = () => {
    checkAndRun('calculator_export', 'retaining-wall-calculator', () => {
      const msg = `*BuildMitra Cantilever RCC Retaining Wall Report*%0A` +
        ...
        `----------------------------------------%0A` +
        `â€¢ *Retaining Wall*: ${inputs.lengthFt}' L x ${inputs.heightFt}' H x ${inputs.thicknessFt}' T (${inputs.grade})%0A` +
        `â€¢ *Base Slab*: ${formatNumber(calcResults.baseWidthFt)}' W x ${formatNumber(calcResults.baseThickFt)}' T%0A` +
        ...
        (calcResults.hasSteel ? `â€¢ *Steel Rebar*: ${formatNumber(calcResults.totalSteelKg)} kg (${inputs.vertDia}mm Vert + ${inputs.horizDia}mm Horiz + ${inputs.toeHeelDia}mm Toe/Heel)%0A` : '') +
        `â€¢ *PVC Weepholes*: ${calcResults.weepholeNos} Nos%0A` +
    ...
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        ...
        <h1 style={styles.headerTitle}>
          ðŸ§± Retaining Wall Structural & Formwork Calculator
      ...
      <MarketRateTrend />
      <EngineeringSpecimen kind="retaining-wall" title="Dynamic Retaining Wall Specimen" material={inputs.grade} data={{ lengthFt: inputs.lengthFt, heightFt: inputs.heightFt, thickness: inputs.thicknessFt, grade: inputs.grade, vertDia: inputs.vertDia, horizDia: inputs.horizDia }} />
      ...
      <div style={styles.dropdownCard}>
        ...
        >
          <option value="both">ðŸ”µ Both Concrete Materials & Steel Rebar (Complete Cantilever RCC Retaining Wall)</option>
          <option value="concrete_only">ðŸ§± Only Concrete Materials & Formwork Shuttering (No Steel Rebar)</option>
          <option value="steel_only">âš™ï¸ Only Steel Rebar & Bar Bending Schedule (Stem & Base Slab Rebar)</option>
        </select>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>ðŸ“ Retaining Wall Geometry & Cantilever Specifications</span>
        </div>

        <div style={styles.noteBox}>
          ðŸ’¡ <strong>IS 456 Retaining Wall Design Standards</strong>: Computes stem and cantilever base slab concrete volume ($B \approx 0.6H$), stem vertical main rebar with $50d$ anchorage, stem horizontal distribution rebar, toe & heel slab mesh, stem front/back formwork shuttering, and 75mm PVC sub-surface drainage weepholes.
        </div>
        ...
        {calcResults.hasSteel && (
          <div style={{ backgroundColor: '#fff5f7', padding: '14px', borderRadius: '8px', marginBottom: '16px', border: '1px solid #fecdd3' }}>
            <div style={{ fontSize: '13px', fontWeight: '700', color: '#800020', marginBottom: '10px' }}>ðŸ”„ Stem & Base Slab Reinforcement (Bar Bending Schedule)</div>
            <div style={styles.grid4}>
              <div style={styles.fieldGroup}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px' }}>
          <button style={styles.btnReset} onClick={handleReset}>ðŸ”„ Reset Form</button>
        </div>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>ðŸ“Š Retaining Wall Results BOQ ({scopeOption === 'both' ? 'Concrete & Steel' : scopeOption === 'concrete_only' ? 'Concrete & Formwork' : 'Steel Rebar Only'})</span>
        </div>
