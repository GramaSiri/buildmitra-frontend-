  ...
  const handleShareWhatsApp = () => {
    checkAndRun('calculator_export', 'roof-truss-calculator', () => {
      const msg = `*BuildMitra Terrace Roof Truss Report*%0A` +
        ...
        `----------------------------------------%0A` +
        `â€¢ *Roof Structure*: ${inputs.lengthFt}' L x ${inputs.widthFt}' Span x ${inputs.riseFt}ft Rise (Terrace Hollow Tube & Round Pipe Columns - ${inputs.roofType})%0A` +
        `â€¢ *Roof Area*: ${formatNumber(calcResults.totalRoofAreaSqft)} Sqft | *Trusses*: ${calcResults.trussNos} Nos%0A` +
        (calcResults.hasFraming ? `â€¢ *Structural Steel Weight*: ${formatNumber(calcResults.totalSteelKg, 1)} kg%0A` : '') +
        `â€¢ *Anti-Corrosive Paint*: ${formatNumber(calcResults.paintAreaSqft, 0)} Sqft (${inputs.paintCoats} Coats)%0A` +
        `â€¢ *Material Total*: ${formatCurrency(calcResults.grandMatCost)}%0A` +
        `â€¢ *Fabrication & Erection Labour*: ${formatCurrency(calcResults.fabLabourCost)}%0A` +
        `â€¢ *TOTAL ESTIMATED COST*: ${formatCurrency(calcResults.grandTotal)} (${formatCurrency(calcResults.costPerSqft)}/Sqft)%0A%0A` +
    ...
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        ...
        <h1 style={styles.headerTitle}>
          ðŸ—ï¸ Terrace Open Roof Truss & Sheeting Calculator
      ...
      <MarketRateTrend />
      <EngineeringSpecimen kind="roof-truss" title="Dynamic Roof Truss Specimen" material={inputs.roofType} data={{ lengthFt: inputs.lengthFt, spanFt: inputs.widthFt, heightFt: inputs.riseFt, trussType: inputs.structureType, type: inputs.roofType }} />
      ...
      <div style={styles.dropdownCard}>
        ...
        >
          <option value="both">ðŸ”µ Both Structural Steel Framing & Roof Sheeting / Tiles (Complete Terrace Roof)</option>
          <option value="sheeting_only">ðŸ§± Only Roof Sheeting / Tiles & Purlins (No Main Structural Framing)</option>
          <option value="framing_only">âš™ï¸ Only Structural Steel Framing & Fabrication (Hollow Tubes & Pipe Columns)</option>
        </select>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>ðŸ“ Terrace Roof Geometry & Structural Specifications</span>
        </div>

        <div style={styles.noteBox}>
          ðŸ’¡ <strong>Terrace Open Structure Standards</strong>: Tailored for 3-side open terrace roofs with air passage. Uses lightweight Square/Rectangular Hollow Section (SHS/RHS) tubes for rafters and web bracing, with MS Round Pipe Columns for light structural weight and maximum durability.
        </div>
        ...
        <div style={styles.grid4}>
          ...
          <div style={styles.fieldGroup}>
            ...
            >
              <option value="Mangalore Tiles">Clay Mangalore Roof Tiles (â‚¹48/sqft)</option>
              <option value="Galvalume Sheets">Metal GI / Galvalume Sheets (â‚¹45/sqft)</option>
              <option value="Polycarbonate">Polycarbonate / Fiber Sheets (â‚¹65/sqft)</option>
            </select>
          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px' }}>
          <button style={styles.btnReset} onClick={handleReset}>ðŸ”„ Reset Terrace Form</button>
        </div>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>ðŸ“Š Terrace Roof Results BOQ ({scopeOption === 'both' ? 'Framing & Sheeting' : scopeOption === 'sheeting_only' ? 'Sheeting Only' : 'Structural Framing Only'})</span>
        </div>
