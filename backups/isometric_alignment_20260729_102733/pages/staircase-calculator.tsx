  ...
  const handleShareWhatsApp = () => {
    checkAndRun('calculator_export', 'staircase-calculator', () => {
      const msg = `*BuildMitra RCC Staircase Structural Report*%0A` +
        ...
        `----------------------------------------%0A` +
        `â€¢ *Floors*: ${inputs.floors} | *Risers*: ${calcResults.totalRisers} Nos (${inputs.riserIn}" Riser x ${inputs.treadIn}" Tread x ${inputs.widthFt}ft Width)%0A` +
        ...
        (calcResults.hasSteel ? `â€¢ *Steel Rebar*: ${formatNumber(calcResults.totalSteelKg)} kg (${inputs.mainDia}mm Main + ${inputs.distDia}mm Dist)%0A` : '') +
        `â€¢ *Finishes & Railing*: ${inputs.finishType} Finish (${formatNumber(calcResults.totalFinishAreaSqft)} Sqft) | ${inputs.railingType} Railing (${formatNumber(calcResults.railingLengthRmt)} RMT)%0A` +
        `â€¢ *Material & Finishes Total*: ${formatCurrency(calcResults.grandMatCost)}%0A` +
    ...
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        ...
        <h1 style={styles.headerTitle}>
          ðŸªœ RCC Staircase Structural & Finishes Calculator
      ...
      <MarketRateTrend />
      <EngineeringSpecimen kind="staircase" title="Dynamic Staircase Specimen" material={inputs.finishType} data={{ heightFt: inputs.floorHeight, widthFt: inputs.widthFt, treadIn: inputs.treadIn, riserIn: inputs.riserIn, finishType: inputs.finishType, railingType: inputs.railingType }} />
      ...
      <div style={styles.dropdownCard}>
        ...
        >
          <option value="both">ðŸ”µ Both Concrete Materials & Steel Rebar (Complete RCC Staircase)</option>
          <option value="concrete_only">ðŸ§± Only Concrete Materials & Formwork (No Steel Rebar)</option>
          <option value="steel_only">âš™ï¸ Only Steel Rebar & Bar Bending Schedule (No Concrete Mix)</option>
        </select>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>ðŸ“ Staircase Geometry & Structural Specifications</span>
        </div>

        <div style={styles.noteBox}>
          ðŸ’¡ <strong>IS 456 Structural Staircase Standards</strong>: Computes waist slab & triangular step concrete, nominal mix proportions, waist main & distribution rebar with $50d$ anchorage, landing steel mat, soffit formwork shuttering, granite/tile step finishes, and MS/SS handrailings.
        </div>
        ...
        {calcResults.hasSteel && (
          <div style={{ backgroundColor: '#fff5f7', padding: '14px', borderRadius: '8px', marginBottom: '16px', border: '1px solid #fecdd3' }}>
            <div style={{ fontSize: '13px', fontWeight: '700', color: '#800020', marginBottom: '10px' }}>ðŸ”„ Waist Slab & Landing Reinforcement (Bar Schedule)</div>
            <div style={styles.grid4}>
              <div style={styles.fieldGroup}>
        ...
        <div style={{ backgroundColor: '#f0fdf4', padding: '14px', borderRadius: '8px', marginBottom: '16px', border: '1px solid #bbf7d0' }}>
          <div style={{ fontSize: '13px', fontWeight: '700', color: '#166534', marginBottom: '10px' }}>ðŸŽ¨ Step Finishes & Handrailings Controls</div>
          <div style={styles.grid3}>
            <div style={styles.fieldGroup}>
              ...
              <select style={styles.select} value={inputs.finishType} onChange={e => setInputs({ ...inputs, finishType: e.target.value })}>
                <option value="Granite">Polished Granite Slabs (â‚¹120/sqft)</option>
                <option value="Tiles">Vitrified Staircase Tiles (â‚¹45/sqft)</option>
              </select>
            </div>
            ...
            <div style={styles.fieldGroup}>
              ...
              <select style={styles.select} value={inputs.railingType} onChange={e => setInputs({ ...inputs, railingType: e.target.value })}>
                <option value="MS">Fabricated MS Railings (â‚¹850/RMT)</option>
                <option value="SS">SS 304 / Toughened Glass Railings (â‚¹1800/RMT)</option>
              </select>
            </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px' }}>
          <button style={styles.btnReset} onClick={handleReset}>ðŸ”„ Reset Staircase Form</button>
        </div>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>ðŸ“Š Staircase Results BOQ ({scopeOption === 'both' ? 'Concrete & Steel' : scopeOption === 'concrete_only' ? 'Concrete & Formwork' : 'Steel Rebar Only'})</span>
        </div>
