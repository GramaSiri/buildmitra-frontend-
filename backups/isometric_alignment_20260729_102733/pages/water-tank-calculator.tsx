  ...
  const handleShareWhatsApp = () => {
    checkAndRun('calculator_export', 'water-tank-calculator', () => {
      const msg = `*BuildMitra Water Storage Tank Structural Report*%0A` +
        ...
        `----------------------------------------%0A` +
        `â€¢ *Tank*: ${inputs.tankNos} Nos (${inputs.lengthFt}'x${inputs.widthFt}' x ${inputs.heightFt}ft Internal - ${inputs.grade})%0A` +
        `â€¢ *Water Capacity*: ${formatNumber(calcResults.capacityLiters)} Litres (${formatNumber(calcResults.internalVolCft)} CFT)%0A` +
        `â€¢ *RCC Concrete Volume*: ${formatNumber(calcResults.rccVolCft)} CFT (${formatNumber(calcResults.rccVolCum, 3)} CUM)%0A` +
        ...
        (calcResults.hasSteel ? `â€¢ *Steel Rebar*: ${formatNumber(calcResults.totalSteelKg)} kg%0A` : '') +
        `â€¢ *Coatings*: Waterproofing & Food Paint (${formatNumber(calcResults.wettedAreaSqft)} Sqft)%0A` +
        `â€¢ *Material & Coatings Total*: ${formatCurrency(calcResults.grandMatCost)}%0A` +
    ...
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        ...
        <h1 style={styles.headerTitle}>
          ðŸ’§ Water Storage Tank Structural & Coatings Calculator
      ...
      <MarketRateTrend />
      <EngineeringSpecimen kind="water-tank" title="Dynamic Water Tank Specimen" material={inputs.wallType} data={{ lengthFt: inputs.lengthFt, widthFt: inputs.widthFt, heightFt: inputs.heightFt, wallType: inputs.wallType, grade: inputs.grade, wallThickIn: inputs.wallThickIn }} />
      ...
      <div style={styles.dropdownCard}>
        ...
        >
          <option value="both">ðŸ”µ Both Concrete Materials & Steel Rebar (Complete RCC Water Tank)</option>
          <option value="concrete_only">ðŸ§± Only Concrete Materials, PCC & Formwork (No Steel Rebar)</option>
          <option value="steel_only">âš™ï¸ Only Steel Rebar Mesh & Cover Slab Bar Schedule (No Concrete Mix)</option>
        </select>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>ðŸ“ Water Tank Geometry & Structural Specifications</span>
        </div>

        <div style={styles.noteBox}>
          ðŸ’¡ <strong>IS Code Water Tank Standards</strong>: Computes net internal storage volume in Litres, RCC base/side wall/cover slab concrete, double mesh rebar schedule, internal liquid waterproofing, non-toxic food grade paint coating, and FRP manhole cover.
        </div>
        ...
        <div style={{ backgroundColor: '#f0fdf4', padding: '14px', borderRadius: '8px', marginBottom: '16px', border: '1px solid #bbf7d0' }}>
          <div style={{ fontSize: '13px', fontWeight: '700', color: '#166534', marginBottom: '10px' }}>ðŸ›¡ï¸ Waterproofing, Food-Grade Paint & Accessories Controls</div>
          <div style={styles.grid3}>
            <div style={styles.fieldGroup}>
              ...
              <select style={styles.select} value={inputs.includeWaterproofing ? "Yes" : "No"} onChange={e => setInputs({ ...inputs, includeWaterproofing: e.target.value === "Yes" })}>
                <option value="Yes">Include Internal Liquid Waterproofing (â‚¹42/sqft)</option>
            ...
            <div style={styles.fieldGroup}>
              ...
              <select style={styles.select} value={inputs.includeFoodPaint ? "Yes" : "No"} onChange={e => setInputs({ ...inputs, includeFoodPaint: e.target.value === "Yes" })}>
                <option value="Yes">Include Non-Toxic Epoxy Food Paint (â‚¹35/sqft)</option>
            ...
            <div style={styles.fieldGroup}>
              ...
              <select style={styles.select} value={inputs.includeFRPManhole ? "Yes" : "No"} onChange={e => setInputs({ ...inputs, includeFRPManhole: e.target.value === "Yes" })}>
                <option value="Yes">Include FRP 2ft x 2ft Manhole Cover (â‚¹1000/nos)</option>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px' }}>
          <button style={styles.btnReset} onClick={handleReset}>ðŸ”„ Reset Water Tank Form</button>
        </div>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>ðŸ“Š Water Tank Results BOQ ({scopeOption === 'both' ? 'Concrete & Steel' : scopeOption === 'concrete_only' ? 'Concrete & Formwork' : 'Steel Rebar Only'})</span>
        </div>
