  ...
  const handleShareWhatsApp = () => {
    checkAndRun('calculator_export', 'brick-work-calculator', () => {
      ...
      const msg = `*BuildMitra Block/Brick Work Calculator Report*%0A` +
        ...
        `----------------------------------------%0A` +
        `â€¢ *Net Wall Area*: ${formatNumber(res.netWallArea)} Sqft%0A` +
        `â€¢ *Masonry Volume*: ${formatNumber(res.masonryVolCft)} CFT%0A` +
        `â€¢ *Bricks / Blocks Quantity*: ${formatNumber(res.totalBlocks, 0)} Nos%0A` +
        `â€¢ *Cement (50kg OPC 53)*: ${formatNumber(res.cemBags)} Bags%0A` +
        `â€¢ *M-Sand*: ${formatNumber(res.sandCft)} CFT%0A` +
        `â€¢ *Site Water*: ${formatNumber(res.waterLtr)} Litres%0A` +
        `â€¢ *Material Cost*: ${formatCurrency(res.grandMatCost)}%0A` +
        `â€¢ *Labour Cost*: ${formatCurrency(res.grandLabCost)}%0A` +
        `â€¢ *TOTAL ESTIMATED COST*: ${formatCurrency(res.grandTotal)} (${formatCurrency(res.costPerSqft)}/Sqft)%0A%0A` +
    ...
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        ...
        <h1 style={styles.headerTitle}>
          ðŸ§± Block/Brick Work Calculator
      ...
      <MarketRateTrend />
      <EngineeringSpecimen kind="masonry" title="Dynamic Brick / Block Specimen" material={calcMode === 'quick' ? quickInputs.blockType : detailedInputs.blockType} data={calcMode === 'quick' ? { length: quickInputs.plotLength, height: 10, wallThicknessInches: quickInputs.internalWallSize.includes('4.5') ? 4.5 : quickInputs.internalWallSize.includes('6') ? 6 : 8, mortarThicknessMm: quickInputs.mortarThicknessMm } : { length: detailedInputs.length, height: detailedInputs.height, wallThicknessInches: detailedInputs.wallThicknessInches, mortarThicknessMm: detailedInputs.mortarThicknessMm }} />
      ...
      <div style={styles.dropdownCard}>
        ...
        >
          ...
          <option value="detailed">ðŸ“ Detailed Wall-Wise Calculation (Exact Room Dimensions & Openings)</option>
        </select>
      </div>
        ...
        <>
          <div style={styles.stepperCard}>
            <div style={styles.sectionHeader}>
              <span>âš¡ Quick Calculation (Plot Dimensions & Wall Thickness BUA Rule)</span>
            </div>

            <div style={styles.noteBox}>
              ðŸ’¡ <strong>IS Code Volumetric Rule</strong>: Block count, Cement Bags, Sand CFT, and Water Litres automatically scale with Wall Thickness (4.5", 6", 8", 9") and Mortar Joint Thickness.
            </div>
          ...
          <div style={styles.stepperCard}>
            <div style={styles.sectionHeader}>
              <span>ðŸ“Š Block/Brick Work Calculation Results & Materials BOQ</span>
            </div>
      ...
      {calcMode === 'detailed' && (
        <div style={styles.stepperCard}>
          <div style={styles.sectionHeader}>
            <span>ðŸ“ Detailed Wall-Wise Brickwork Calculation Inputs (IS 2212 Exact Rules)</span>
          </div>
          ...
          <div style={{ backgroundColor: '#f1f5f9', padding: '12px', borderRadius: '8px', marginBottom: '16px' }}>
            <div style={{ fontSize: '13px', fontWeight: '700', color: '#0f766e', marginBottom: '8px' }}>ðŸšª Openings Deduction (Doors & Windows Volume Deduction)</div>
            <div style={styles.grid4}>
              <div style={styles.fieldGroup}>
          ...
          <div style={{ marginTop: '16px' }}>
            <div style={styles.sectionHeader}>
              <span>ðŸ“Š Detailed Block/Brick Work Calculation Results & Materials BOQ</span>
            </div>
