  ...
  const handleShareWhatsApp = () => {
    checkAndRun('calculator_export', 'rcc-steel-building-calculator', () => {
      const msg = `*BuildMitra Full Building Construction Estimate Report*%0A` +
        `*Estimation Mode*: ${calcMode === 'quick' ? 'âš¡ Quick Civil Shell' : 'ðŸ”‘ Turnkey Full Finished Building'}%0A` +
        `----------------------------------------%0A` +
        `â€¢ *Total Built-Up Area (BUA)*: ${formatNumber(calcResults.bua)} Sqft (${inputs.floors} Floors)%0A` +
        `â€¢ *Cement (50kg OPC 53)*: ${formatNumber(calcResults.cementBags)} Bags%0A` +
        `â€¢ *TMT Steel Rebar (Fe 500D)*: ${formatNumber(calcResults.totalSteelKg)} kg%0A` +
        `â€¢ *Masonry Blocks*: 6" Outer ${formatNumber(calcResults.block6Nos, 0)} Nos | 4.5" Partition ${formatNumber(calcResults.block4Nos, 0)} Nos%0A` +
        `----------------------------------------%0A` +
        `â€¢ *âš¡ Quick Civil Shell Cost*: ${formatCurrency(calcResults.quickGrandTotal)} (${formatCurrency(calcResults.quickRatePerSqft)}/Sqft)%0A` +
        `â€¢ *ðŸ”‘ Turnkey Full Finished Cost*: ${formatCurrency(calcResults.turnkeyGrandTotal)} (${formatCurrency(calcResults.turnkeyRatePerSqft)}/Sqft)%0A%0A` +
    ...
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        ...
        <h1 style={styles.headerTitle}>
          ðŸ—ï¸ Full Building Structural & Turnkey Finishes Calculator
      ...
      <div style={styles.dropdownCard}>
        ...
        >
          <option value="quick">âš¡ Quick Civil Shell Calculation (Structure + Masonry + Plastering + Civil Labour)</option>
          <option value="turnkey">ðŸ”‘ Turnkey Full Finished Building Calculation (Complete Shell + Tiles + Sanitary + Doors + Painting + Gates + Wiring + Tanks)</option>
        </select>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>ðŸ“ Building Plot & Floor Dimensions</span>
        </div>

        <div style={styles.noteBox}>
          ðŸ’¡ <strong>Consolidated Civil Engineering Benchmark</strong>: Enter Plot Dimensions & Floors. System calculates consolidated quantities for Structure, Masonry, Plaster, Doors/Windows, MEP, Plumbing, Electrical, Compound Wall, Tanks, and Turnkey Labour.
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px' }}>
          <button style={styles.btnReset} onClick={handleReset}>ðŸ”„ Reset Building Form</button>
        </div>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>ðŸ“Š Full Building Consolidated Results & Cost Comparisons</span>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '14px', marginBottom: '18px' }}>
          <div style={{ backgroundColor: '#f0fdf4', border: '2px solid #bbf7d0', borderRadius: '10px', padding: '16px', color: '#166534' }}>
            <div style={{ fontSize: '12px', fontWeight: '800', textTransform: 'uppercase' }}>âš¡ Quick Civil Shell Estimate</div>
          ...
          <div style={{ backgroundColor: '#fff5f7', border: '2px solid #fecdd3', borderRadius: '10px', padding: '16px', color: '#800020' }}>
            <div style={{ fontSize: '12px', fontWeight: '800', textTransform: 'uppercase' }}>ðŸ”‘ Turnkey Full Finished Building</div>
