  ...
  const handleShareWhatsApp = () => {
    checkAndRun('calculator_export', 'lintel-calculator', () => {
      const msg = `*BuildMitra RCC Lintel Beam Report*%0A` +
        ...
        `----------------------------------------%0A` +
        `â€¢ *Lintels*: ${inputs.lintelNos} Nos (${inputs.clearLenFt}' Clear Span x ${inputs.widthIn}" W x ${inputs.depthIn}" D - ${inputs.grade})%0A` +
        `â€¢ *Concrete Volume*: ${formatNumber(calcResults.totalVolCft)} CFT (${formatNumber(calcResults.totalVolCum, 3)} CUM)%0A` +
        (calcResults.hasConcrete ? `â€¢ *Cement*: ${formatNumber(calcResults.cementBags)} Bags | *Shuttering*: ${formatNumber(calcResults.shutteringAreaSqft)} Sqft%0A` : '') +
        (calcResults.hasSteel ? `â€¢ *Steel Rebar*: ${formatNumber(calcResults.totalSteelKg)} kg (${inputs.mainDia}mm Main + ${inputs.stirrupDia}mm Stirrups)%0A` : '') +
        `â€¢ *Material Total*: ${formatCurrency(calcResults.grandMatCost)}%0A` +
        `â€¢ *Labour Total*: ${formatCurrency(calcResults.rccLabourCost)}%0A` +
        `â€¢ *TOTAL ESTIMATED COST*: ${formatCurrency(calcResults.grandTotal)} (${formatCurrency(calcResults.costPerCft)}/CFT)%0A%0A` +
    ...
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        ...
        <h1 style={styles.headerTitle}>
          ðŸšª Lintel Beam Structural & Formwork Calculator
      ...
      <MarketRateTrend />
      <EngineeringSpecimen kind="lintel" title="Dynamic Lintel Specimen" material={inputs.grade} data={{ lengthFt: inputs.clearLenFt, clearLenFt: inputs.clearLenFt, widthIn: inputs.widthIn, depthIn: inputs.depthIn, grade: inputs.grade, bearingMm: inputs.bearingMm, mainDia: inputs.mainDia }} />
      ...
      <div style={styles.dropdownCard}>
        ...
        >
          <option value="both">ðŸ”µ Both Concrete Materials & Steel Rebar (Complete RCC Lintel Beam)</option>
          <option value="concrete_only">ðŸ§± Only Concrete Materials & Formwork Shuttering (No Steel Rebar)</option>
          <option value="steel_only">âš™ï¸ Only Steel Rebar & 2-Legged Stirrups (Bar Bending Schedule)</option>
        </select>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>ðŸ“ Lintel Geometry & Bearing Specifications</span>
        </div>

        <div style={styles.noteBox}>
          ðŸ’¡ <strong>IS 456 Lintel Beam Design Standards</strong>: Computes clear span opening plus side wall bearing (default 150mm each side), total concrete volume, bottom soffit and side formwork shuttering, main longitudinal bars with $50d$ anchorage, and 2-legged 8mm stirrups.
        </div>
        ...
        {calcResults.hasSteel && (
          <div style={{ backgroundColor: '#fff5f7', padding: '14px', borderRadius: '8px', marginBottom: '16px', border: '1px solid #fecdd3' }}>
            <div style={{ fontSize: '13px', fontWeight: '700', color: '#800020', marginBottom: '10px' }}>ðŸ”„ Lintel Reinforcement & Stirrups (Bar Bending Schedule)</div>
            <div style={styles.grid4}>
              <div style={styles.fieldGroup}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px' }}>
          <button style={styles.btnReset} onClick={handleReset}>ðŸ”„ Reset Lintel Form</button>
        </div>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>ðŸ“Š Lintel Beam Results BOQ ({scopeOption === 'both' ? 'Concrete & Steel' : scopeOption === 'concrete_only' ? 'Concrete & Formwork' : 'Steel Rebar Only'})</span>
        </div>
