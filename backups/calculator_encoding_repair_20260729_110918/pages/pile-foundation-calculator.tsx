  ...
  const handleShareWhatsApp = () => {
    checkAndRun('calculator_export', 'pile-foundation-calculator', () => {
      const msg = `*BuildMitra RCC Pile Foundation Report*%0A` +
        ...
        `----------------------------------------%0A` +
        `Ã¢â‚¬Â¢ *Piles*: ${inputs.pileNos} Nos (${inputs.diameterFt}' Dia x ${inputs.lengthFt}ft Depth - ${inputs.grade})%0A` +
        `Ã¢â‚¬Â¢ *Auger Pit Boring*: ${formatNumber(calcResults.boringLenM)} RMT%0A` +
        `Ã¢â‚¬Â¢ *Concrete Volume*: ${formatNumber(calcResults.totalVolCft)} CFT (${formatNumber(calcResults.totalVolCum, 3)} CUM)%0A` +
        (calcResults.hasConcrete ? `Ã¢â‚¬Â¢ *Cement*: ${formatNumber(calcResults.cementBags)} Bags%0A` : '') +
        (calcResults.hasSteel ? `Ã¢â‚¬Â¢ *Steel Rebar Cage*: ${formatNumber(calcResults.totalSteelKg)} kg (${inputs.mainDia}mm Main + ${inputs.tieDia}mm Ties)%0A` : '') +
        `Ã¢â‚¬Â¢ *Material & Boring Total*: ${formatCurrency(calcResults.grandMatCost)}%0A` +
...
{/* 1. Header */}
      <div style={styles.header}>
        ...
        <h1 style={styles.headerTitle}>
          Ã°Å¸Â¥Â§ Pile Foundation Structural & Boring Calculator
      ...
      <div style={styles.dropdownCard}>
        ...
        >
          <option value="both">Ã°Å¸â€Âµ Both Concrete Materials, Boring & Steel Rebar (Complete RCC Pile Foundation)</option>
          <option value="boring_only">Ã°Å¸Â§Â± Only Concrete Materials & Auger Pit Boring (No Steel Rebar Cage)</option>
          <option value="cage_only">Ã¢Å¡â„¢Ã¯Â¸Â Only Steel Rebar Cage & Circular Helical Ties (Bar Bending Schedule)</option>
        </select>
      </div>
        </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>Ã°Å¸â€œÂ Pile Geometry & Boring Specifications</span>
        </div>

        <div style={styles.noteBox}>
          Ã°Å¸â€™Â¡ <strong>IS 2911 Pile Foundation Design Standards</strong>: Computes auger pit boring depth in RMT, tremie concrete volume, main longitudinal bar schedule with $50d$ anchorage, circular helical ties/rings, and heavy duty circular wheel spacers (50mm cover).
        </div>
        ...
        {calcResults.hasSteel && (
          <div style={{ backgroundColor: '#fff5f7', padding: '14px', borderRadius: '8px', marginBottom: '16px', border: '1px solid #fecdd3' }}>
            <div style={{ fontSize: '13px', fontWeight: '700', color: '#800020', marginBottom: '10px' }}>Ã°Å¸â€â€ž Reinforcement Cage & Circular Helical Ties (Bar Bending Schedule)</div>
            <div style={styles.grid4}>
              <div style={styles.fieldGroup}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px' }}>
          <button style={styles.btnReset} onClick={handleReset}>Ã°Å¸â€â€ž Reset Pile Form</button>
        </div>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>Ã°Å¸â€œÅ  Pile Foundation Results BOQ ({scopeOption === 'both' ? 'Concrete & Steel' : scopeOption === 'boring_only' ? 'Concrete & Boring' : 'Steel Cage Only'})</span>
        </div>
