  ...
  const handleShareWhatsApp = () => {
    checkAndRun('calculator_export', 'septic-tank-calculator', () => {
      const msg = `*BuildMitra Septic Tank Structural Report*%0A` +
        `----------------------------------------%0A` +
        `Ã¢â‚¬Â¢ *Septic Tank*: ${inputs.lengthFt}'x${inputs.widthFt}' x ${inputs.depthFt}ft Internal (${inputs.wallMaterial} in 1:20 Lean Mortar)%0A` +
        `Ã¢â‚¬Â¢ *Effective Capacity*: ${formatNumber(calcResults.effectiveCapacityLiters)} Litres (~${calcResults.userCapacityNos} Users)%0A` +
        `Ã¢â‚¬Â¢ *200mm RCC Cover Slab*: ${formatNumber(calcResults.coverSlabVolCft)} CFT (${formatNumber(calcResults.coverSlabVolCum, 3)} CUM)%0A` +
        `Ã¢â‚¬Â¢ *Minimal Cement Total*: ${formatNumber(calcResults.totalCementBags, 1)} Bags (Wall Mortar: ${formatNumber(calcResults.cementBagsWall, 1)} Bags)%0A` +
        `Ã¢â‚¬Â¢ *Cover Slab Steel Rebar*: ${formatNumber(calcResults.coverSteelKg, 1)} kg%0A` +
        `Ã¢â‚¬Â¢ *Plumbing*: PVC Inlet, Outlet & 100mm Air Vent Set%0A` +
...
{/* 1. Header */}
      <div style={styles.header}>
        ...
        <h1 style={styles.headerTitle}>
          Ã°Å¸Å¡Â½ Septic Tank Volumetric & Structural Calculator
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>Ã°Å¸â€œÂ Septic Tank Geometry, Masonry & Cover Slab Specifications</span>
        </div>

        <div style={styles.noteBox}>
          Ã°Å¸â€™Â¡ <strong>IS 2470 Septic Tank Code Standards</strong>: Recommends Size Stones or Concrete Blocks laid in 1:20 lean mortar ratio to minimize cement usage (no heavy PCC bed). Includes 200mm (8") RCC cover slab, PVC inlet/outlet sleeves, and 100mm air vent pipe set.
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px' }}>
          <button style={styles.btnReset} onClick={handleReset}>Ã°Å¸â€â€ž Reset Septic Tank Form</button>
        </div>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>Ã°Å¸â€œÅ  Septic Tank Results BOQ (Size Stones / Blocks + 200mm Cover Slab)</span>
        </div>
