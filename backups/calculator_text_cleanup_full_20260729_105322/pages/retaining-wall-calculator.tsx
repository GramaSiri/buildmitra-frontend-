  ...
  const handleShareWhatsApp = () => {
    checkAndRun('calculator_export', 'retaining-wall-calculator', () => {
      const msg = `*BuildMitra Cantilever RCC Retaining Wall Report*%0A` +
        ...
        `----------------------------------------%0A` +
        `Ã¢â‚¬Â¢ *Retaining Wall*: ${inputs.lengthFt}' L x ${inputs.heightFt}' H x ${inputs.thicknessFt}' T (${inputs.grade})%0A` +
        `Ã¢â‚¬Â¢ *Base Slab*: ${formatNumber(calcResults.baseWidthFt)}' W x ${formatNumber(calcResults.baseThickFt)}' T%0A` +
        ...
        (calcResults.hasSteel ? `Ã¢â‚¬Â¢ *Steel Rebar*: ${formatNumber(calcResults.totalSteelKg)} kg (${inputs.vertDia}mm Vert + ${inputs.horizDia}mm Horiz + ${inputs.toeHeelDia}mm Toe/Heel)%0A` : '') +
        `Ã¢â‚¬Â¢ *PVC Weepholes*: ${calcResults.weepholeNos} Nos%0A` +
...
{/* 1. Header */}
      <div style={styles.header}>
        ...
        <h1 style={styles.headerTitle}>
          Ã°Å¸Â§Â± Retaining Wall Structural & Formwork Calculator
      ...
      <div style={styles.dropdownCard}>
        ...
        >
          <option value="both">Ã°Å¸â€Âµ Both Concrete Materials & Steel Rebar (Complete Cantilever RCC Retaining Wall)</option>
          <option value="concrete_only">Ã°Å¸Â§Â± Only Concrete Materials & Formwork Shuttering (No Steel Rebar)</option>
          <option value="steel_only">Ã¢Å¡â„¢Ã¯Â¸Â Only Steel Rebar & Bar Bending Schedule (Stem & Base Slab Rebar)</option>
        </select>
      </div>
        </div>
      ...
      <div style={styles.stepperCard}>
        ...
        {calcResults.hasSteel && (
          <div style={{ backgroundColor: '#fff5f7', padding: '14px', borderRadius: '8px', marginBottom: '16px', border: '1px solid #fecdd3' }}>
            <div style={{ fontSize: '13px', fontWeight: '700', color: '#800020', marginBottom: '10px' }}>Ã°Å¸â€â€ž Stem & Base Slab Reinforcement (Bar Bending Schedule)</div>
            <div style={styles.grid4}>
              <div style={styles.fieldGroup}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px' }}>
          <button style={styles.btnReset} onClick={handleReset}>Ã°Å¸â€â€ž Reset Form</button>
        </div>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>Ã°Å¸â€œÅ  Retaining Wall Results BOQ ({scopeOption === 'both' ? 'Concrete & Steel' : scopeOption === 'concrete_only' ? 'Concrete & Formwork' : 'Steel Rebar Only'})</span>
        </div>
