  ...
  const handleShareWhatsApp = () => {
    checkAndRun('calculator_export', 'water-tank-calculator', () => {
      const msg = `*BuildMitra Water Storage Tank Structural Report*%0A` +
        ...
        `----------------------------------------%0A` +
        `Ã¢â‚¬Â¢ *Tank*: ${inputs.tankNos} Nos (${inputs.lengthFt}'x${inputs.widthFt}' x ${inputs.heightFt}ft Internal - ${inputs.grade})%0A` +
        `Ã¢â‚¬Â¢ *Water Capacity*: ${formatNumber(calcResults.capacityLiters)} Litres (${formatNumber(calcResults.internalVolCft)} CFT)%0A` +
        `Ã¢â‚¬Â¢ *RCC Concrete Volume*: ${formatNumber(calcResults.rccVolCft)} CFT (${formatNumber(calcResults.rccVolCum, 3)} CUM)%0A` +
        ...
        (calcResults.hasSteel ? `Ã¢â‚¬Â¢ *Steel Rebar*: ${formatNumber(calcResults.totalSteelKg)} kg%0A` : '') +
        `Ã¢â‚¬Â¢ *Coatings*: Waterproofing & Food Paint (${formatNumber(calcResults.wettedAreaSqft)} Sqft)%0A` +
        `Ã¢â‚¬Â¢ *Material & Coatings Total*: ${formatCurrency(calcResults.grandMatCost)}%0A` +
        `Ã¢â‚¬Â¢ *Labour Total*: ${formatCurrency(calcResults.rccLabourCost)}%0A` +
        `Ã¢â‚¬Â¢ *TOTAL ESTIMATED COST*: ${formatCurrency(calcResults.grandTotal)} (${formatCurrency(calcResults.costPerLitre)}/Litre)%0A%0A` +
...
{/* 1. Header */}
      <div style={styles.header}>
        ...
        <h1 style={styles.headerTitle}>
          Ã°Å¸â€™Â§ Water Storage Tank Structural & Coatings Calculator
      ...
      <div style={styles.dropdownCard}>
        ...
        >
          <option value="both">Ã°Å¸â€Âµ Both Concrete Materials & Steel Rebar (Complete RCC Water Tank)</option>
          <option value="concrete_only">Ã°Å¸Â§Â± Only Concrete Materials, PCC & Formwork (No Steel Rebar)</option>
          <option value="steel_only">Ã¢Å¡â„¢Ã¯Â¸Â Only Steel Rebar Mesh & Cover Slab Bar Schedule (No Concrete Mix)</option>
        </select>
      </div>
        </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>Ã°Å¸â€œÂ Water Tank Geometry & Structural Specifications</span>
        </div>

        <div style={styles.noteBox}>
          Ã°Å¸â€™Â¡ <strong>IS Code Water Tank Standards</strong>: Computes net internal storage volume in Litres, RCC base/side wall/cover slab concrete, double mesh rebar schedule, internal liquid waterproofing, non-toxic food grade paint coating, and FRP manhole cover.
        </div>
        ...
        <div style={{ backgroundColor: '#f0fdf4', padding: '14px', borderRadius: '8px', marginBottom: '16px', border: '1px solid #bbf7d0' }}>
          <div style={{ fontSize: '13px', fontWeight: '700', color: '#166534', marginBottom: '10px' }}>Ã°Å¸â€ºÂ¡Ã¯Â¸Â Waterproofing, Food-Grade Paint & Accessories Controls</div>
          <div style={styles.grid3}>
            <div style={styles.fieldGroup}>
              ...
              <select style={styles.select} value={inputs.includeWaterproofing ? "Yes" : "No"} onChange={e => setInputs({ ...inputs, includeWaterproofing: e.target.value === "Yes" })}>
                <option value="Yes">Include Internal Liquid Waterproofing (Ã¢â€šÂ¹42/sqft)</option>
            ...
            <div style={styles.fieldGroup}>
              ...
              <select style={styles.select} value={inputs.includeFoodPaint ? "Yes" : "No"} onChange={e => setInputs({ ...inputs, includeFoodPaint: e.target.value === "Yes" })}>
                <option value="Yes">Include Non-Toxic Epoxy Food Paint (Ã¢â€šÂ¹35/sqft)</option>
            ...
            <div style={styles.fieldGroup}>
              ...
              <select style={styles.select} value={inputs.includeFRPManhole ? "Yes" : "No"} onChange={e => setInputs({ ...inputs, includeFRPManhole: e.target.value === "Yes" })}>
                <option value="Yes">Include FRP 2ft x 2ft Manhole Cover (Ã¢â€šÂ¹1000/nos)</option>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px' }}>
          <button style={styles.btnReset} onClick={handleReset}>Ã°Å¸â€â€ž Reset Water Tank Form</button>
        </div>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>Ã°Å¸â€œÅ  Water Tank Results BOQ ({scopeOption === 'both' ? 'Concrete & Steel' : scopeOption === 'concrete_only' ? 'Concrete & Formwork' : 'Steel Rebar Only'})</span>
        </div>
