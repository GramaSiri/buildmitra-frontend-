  ...
  const handleShareWhatsApp = () => {
    checkAndRun('calculator_export', 'staircase-calculator', () => {
      const msg = `*BuildMitra RCC Staircase Structural Report*%0A` +
        ...
        `----------------------------------------%0A` +
        `Ã¢â‚¬Â¢ *Floors*: ${inputs.floors} | *Risers*: ${calcResults.totalRisers} Nos (${inputs.riserIn}" Riser x ${inputs.treadIn}" Tread x ${inputs.widthFt}ft Width)%0A` +
        ...
        (calcResults.hasSteel ? `Ã¢â‚¬Â¢ *Steel Rebar*: ${formatNumber(calcResults.totalSteelKg)} kg (${inputs.mainDia}mm Main + ${inputs.distDia}mm Dist)%0A` : '') +
        `Ã¢â‚¬Â¢ *Finishes & Railing*: ${inputs.finishType} Finish (${formatNumber(calcResults.totalFinishAreaSqft)} Sqft) | ${inputs.railingType} Railing (${formatNumber(calcResults.railingLengthRmt)} RMT)%0A` +
        `Ã¢â‚¬Â¢ *Material & Finishes Total*: ${formatCurrency(calcResults.grandMatCost)}%0A` +
...
{/* 1. Header */}
      <div style={styles.header}>
        ...
        <h1 style={styles.headerTitle}>
          Ã°Å¸ÂªÅ“ RCC Staircase Structural & Finishes Calculator
      ...
      <div style={styles.dropdownCard}>
        ...
        >
          <option value="both">Ã°Å¸â€Âµ Both Concrete Materials & Steel Rebar (Complete RCC Staircase)</option>
          <option value="concrete_only">Ã°Å¸Â§Â± Only Concrete Materials & Formwork (No Steel Rebar)</option>
          <option value="steel_only">Ã¢Å¡â„¢Ã¯Â¸Â Only Steel Rebar & Bar Bending Schedule (No Concrete Mix)</option>
        </select>
      </div>
        </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>Ã°Å¸â€œÂ Staircase Geometry & Structural Specifications</span>
        </div>

        <div style={styles.noteBox}>
          Ã°Å¸â€™Â¡ <strong>IS 456 Structural Staircase Standards</strong>: Computes waist slab & triangular step concrete, nominal mix proportions, waist main & distribution rebar with $50d$ anchorage, landing steel mat, soffit formwork shuttering, granite/tile step finishes, and MS/SS handrailings.
        </div>
        ...
        {calcResults.hasSteel && (
          <div style={{ backgroundColor: '#fff5f7', padding: '14px', borderRadius: '8px', marginBottom: '16px', border: '1px solid #fecdd3' }}>
            <div style={{ fontSize: '13px', fontWeight: '700', color: '#800020', marginBottom: '10px' }}>Ã°Å¸â€â€ž Waist Slab & Landing Reinforcement (Bar Schedule)</div>
            <div style={styles.grid4}>
              <div style={styles.fieldGroup}>
        ...
        <div style={{ backgroundColor: '#f0fdf4', padding: '14px', borderRadius: '8px', marginBottom: '16px', border: '1px solid #bbf7d0' }}>
          <div style={{ fontSize: '13px', fontWeight: '700', color: '#166534', marginBottom: '10px' }}>Ã°Å¸Å½Â¨ Step Finishes & Handrailings Controls</div>
          <div style={styles.grid3}>
            <div style={styles.fieldGroup}>
              ...
              <select style={styles.select} value={inputs.finishType} onChange={e => setInputs({ ...inputs, finishType: e.target.value })}>
                <option value="Granite">Polished Granite Slabs (Ã¢â€šÂ¹120/sqft)</option>
                <option value="Tiles">Vitrified Staircase Tiles (Ã¢â€šÂ¹45/sqft)</option>
              </select>
            </div>
            ...
            <div style={styles.fieldGroup}>
              ...
              <select style={styles.select} value={inputs.railingType} onChange={e => setInputs({ ...inputs, railingType: e.target.value })}>
                <option value="MS">Fabricated MS Railings (Ã¢â€šÂ¹850/RMT)</option>
                <option value="SS">SS 304 / Toughened Glass Railings (Ã¢â€šÂ¹1800/RMT)</option>
              </select>
            </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px' }}>
          <button style={styles.btnReset} onClick={handleReset}>Ã°Å¸â€â€ž Reset Staircase Form</button>
        </div>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>Ã°Å¸â€œÅ  Staircase Results BOQ ({scopeOption === 'both' ? 'Concrete & Steel' : scopeOption === 'concrete_only' ? 'Concrete & Formwork' : 'Steel Rebar Only'})</span>
        </div>
