  ...
  const handleShareWhatsApp = () => {
    checkAndRun('calculator_export', 'roof-truss-calculator', () => {
      const msg = `*BuildMitra Terrace Roof Truss Report*%0A` +
        ...
        `----------------------------------------%0A` +
        `Ã¢â‚¬Â¢ *Roof Structure*: ${inputs.lengthFt}' L x ${inputs.widthFt}' Span x ${inputs.riseFt}ft Rise (Terrace Hollow Tube & Round Pipe Columns - ${inputs.roofType})%0A` +
        `Ã¢â‚¬Â¢ *Roof Area*: ${formatNumber(calcResults.totalRoofAreaSqft)} Sqft | *Trusses*: ${calcResults.trussNos} Nos%0A` +
        (calcResults.hasFraming ? `Ã¢â‚¬Â¢ *Structural Steel Weight*: ${formatNumber(calcResults.totalSteelKg, 1)} kg%0A` : '') +
        `Ã¢â‚¬Â¢ *Anti-Corrosive Paint*: ${formatNumber(calcResults.paintAreaSqft, 0)} Sqft (${inputs.paintCoats} Coats)%0A` +
        `Ã¢â‚¬Â¢ *Material Total*: ${formatCurrency(calcResults.grandMatCost)}%0A` +
        `Ã¢â‚¬Â¢ *Fabrication & Erection Labour*: ${formatCurrency(calcResults.fabLabourCost)}%0A` +
        `Ã¢â‚¬Â¢ *TOTAL ESTIMATED COST*: ${formatCurrency(calcResults.grandTotal)} (${formatCurrency(calcResults.costPerSqft)}/Sqft)%0A%0A` +
...
{/* 1. Header */}
      <div style={styles.header}>
        ...
        <h1 style={styles.headerTitle}>
          Ã°Å¸Ââ€”Ã¯Â¸Â Terrace Open Roof Truss & Sheeting Calculator
      ...
      <div style={styles.dropdownCard}>
        ...
        >
          <option value="both">Ã°Å¸â€Âµ Both Structural Steel Framing & Roof Sheeting / Tiles (Complete Terrace Roof)</option>
          <option value="sheeting_only">Ã°Å¸Â§Â± Only Roof Sheeting / Tiles & Purlins (No Main Structural Framing)</option>
          <option value="framing_only">Ã¢Å¡â„¢Ã¯Â¸Â Only Structural Steel Framing & Fabrication (Hollow Tubes & Pipe Columns)</option>
        </select>
      </div>
        </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>Ã°Å¸â€œÂ Terrace Roof Geometry & Structural Specifications</span>
        </div>

        <div style={styles.noteBox}>
          Ã°Å¸â€™Â¡ <strong>Terrace Open Structure Standards</strong>: Tailored for 3-side open terrace roofs with air passage. Uses lightweight Square/Rectangular Hollow Section (SHS/RHS) tubes for rafters and web bracing, with MS Round Pipe Columns for light structural weight and maximum durability.
        </div>
        ...
        <div style={styles.grid4}>
          ...
          <div style={styles.fieldGroup}>
            ...
            >
              <option value="Mangalore Tiles">Clay Mangalore Roof Tiles (Ã¢â€šÂ¹48/sqft)</option>
              <option value="Galvalume Sheets">Metal GI / Galvalume Sheets (Ã¢â€šÂ¹45/sqft)</option>
              <option value="Polycarbonate">Polycarbonate / Fiber Sheets (Ã¢â€šÂ¹65/sqft)</option>
            </select>
          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px' }}>
          <button style={styles.btnReset} onClick={handleReset}>Ã°Å¸â€â€ž Reset Terrace Form</button>
        </div>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>Ã°Å¸â€œÅ  Terrace Roof Results BOQ ({scopeOption === 'both' ? 'Framing & Sheeting' : scopeOption === 'sheeting_only' ? 'Sheeting Only' : 'Structural Framing Only'})</span>
        </div>
