  ...
  const handleShareWhatsApp = () => {
    checkAndRun('calculator_export', 'footing-calculator', () => {
      const msg = `*BuildMitra RCC Footing Structural Report*%0A` +
        ...
        `----------------------------------------%0A` +
        `Ã¢â‚¬Â¢ *Footings*: ${inputs.footingNos} Nos (${inputs.lengthFt}'x${inputs.widthFt}' x ${inputs.depthFt}ft - ${inputs.grade})%0A` +
        `Ã¢â‚¬Â¢ *Excavation Volume*: ${formatNumber(calcResults.excVolCum)} CUM | *PCC Volume*: ${formatNumber(calcResults.pccVolCum)} CUM%0A` +
        `Ã¢â‚¬Â¢ *RCC Concrete Volume*: ${formatNumber(calcResults.rccVolCft)} CFT (${formatNumber(calcResults.rccVolCum, 3)} CUM)%0A` +
        (calcResults.hasConcrete ? `Ã¢â‚¬Â¢ *Cement*: ${formatNumber(calcResults.totalCementBags)} Bags | *Shuttering*: ${formatNumber(calcResults.shutteringAreaSqft)} Sqft%0A` : '') +
        ...
        `Ã¢â‚¬Â¢ *Material & Formwork Total*: ${formatCurrency(calcResults.grandMatCost)}%0A` +
...
{/* 1. Header */}
      <div style={styles.header}>
        ...
        <h1 style={styles.headerTitle}>
          Ã°Å¸Ââ€”Ã¯Â¸Â RCC Footing Structural & Formwork Calculator
      ...
      <div style={styles.dropdownCard}>
        ...
        >
          <option value="both">Ã°Å¸â€Âµ Both Concrete Materials, PCC & Steel Rebar (Complete RCC Footing)</option>
          <option value="concrete_only">Ã°Å¸Â§Â± Only Concrete Materials, PCC Bed & Shuttering (No Steel Rebar)</option>
          <option value="steel_only">Ã¢Å¡â„¢Ã¯Â¸Â Only Steel Rebar & Footing Mesh Mat (No Concrete Mix)</option>
        </select>
      </div>
        </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>Ã°Å¸â€œÂ Footing Geometry, Excavation & PCC Specifications</span>
        </div>

        <div style={styles.noteBox}>
          Ã°Å¸â€™Â¡ <strong>IS 456 Structural Footing Standards</strong>: Select whether to compute Concrete Materials, Steel Rebar, or Both. Includes pit excavation volume, PCC 1:4:8 bed, nominal mix proportions, mesh mat bar schedule with L-bends, 50mm footing cover, and 4-side formwork shuttering.
        </div>
        ...
        {calcResults.hasSteel && (
          <div style={{ backgroundColor: '#fff5f7', padding: '14px', borderRadius: '8px', marginBottom: '16px', border: '1px solid #fecdd3' }}>
            <div style={{ fontSize: '13px', fontWeight: '700', color: '#800020', marginBottom: '10px' }}>Ã°Å¸â€â€ž Footing Mesh Mat Reinforcement (Bar Schedule)</div>
            <div style={styles.grid4}>
              <div style={styles.fieldGroup}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px' }}>
          <button style={styles.btnReset} onClick={handleReset}>Ã°Å¸â€â€ž Reset Footing Form</button>
        </div>
      </div>
      ...
      <div style={styles.stepperCard}>
        <div style={styles.sectionHeader}>
          <span>Ã°Å¸â€œÅ  Footing Results BOQ ({scopeOption === 'both' ? 'Concrete & Steel' : scopeOption === 'concrete_only' ? 'Concrete & Formwork' : 'Steel Rebar Only'})</span>
        </div>
