...
export default function Marketplace() {
  ...
  const serviceTypes = ["All", "Suppliers", "Contractors", "Machinery Hire", "Labour Supply", "Real Estate"];
  ...
  const normalizeItem = (raw: any, sourceKey: string, index: number): MarketItem | null => {
    ...
    if (key.includes("machine") || key.includes("machinery") || raw.equipment || raw.machineName || raw.dailyRate || raw.rent) type = "Machinery Hire";
    if (key.includes("labour") || raw.labourName || raw.trade || raw.workerCount) type = "Labour Supply";
    if (key.includes("realestate") || key.includes("real_estate") || raw.propertyType || raw.propertyName) type = "Real Estate";
    if (key.includes("supplier") || raw.productName || raw.materialName || raw.material) type = "Suppliers";
  ...
  const serviceAllowed = (item: MarketItem) => {
    ...
    if (Array.isArray(adminPins) && adminPins.length > 0) {
      return adminPins.some((p: any) =>
        String(p.pincode || p.pin || p).trim() === pincode.trim() &&
        (!p.sourceType || p.sourceType === item.sourceType || p.sourceType === "All")
      );
    }
  ...
  const filtered = items.filter((item) => {
    ...
    return matchSearch && matchType && serviceAllowed(item);
  });
  ...
  return (
    <div style={{ padding: 20, background: "#f3f4f6", minHeight: "100vh", fontFamily: "Arial" }}>
      ...
      <div style={{ background: "white", padding: 16, borderRadius: 12, marginBottom: 20, display: "grid", gridTemplateColumns: "2fr 1fr 1fr auto", gap: 12 }}>
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search item, supplier name, code, location..." style={{ padding: 10, border: "1px solid #ddd", borderRadius: 8 }} />
        ...
        <input value={pincode} onChange={(e) => setPincode(e.target.value)} placeholder="Service pincode" style={{ padding: 10, border: "1px solid #ddd", borderRadius: 8 }} />
