...
const readUsers = () => readArray("users");
...
export default function Marketplace() {
  ...
  const findUploader = (raw: any, type: string) => {
    ...
    const roleMap: any = {
      Products: ["supplier", "vendor"],
      Contractors: ["contractor"],
      Labours: ["labour", "laboursupply"],
      "Machine Rentals": ["machinery", "machinehire"],
    ...
    const matched =
      users.find((u: any) => possibleCode && text(u.uniqueCode).toUpperCase() === possibleCode.toUpperCase()) ||
      users.find((u: any) => possibleMobile && text(u.mobile || u.phone).replace(/[^0-9]/g, "") === possibleMobile.replace(/[^0-9]/g, "")) ||
      users.find((u: any) => possibleName && text(u.name).toLowerCase() === possibleName.toLowerCase() && (!roles.length || roles.includes(text(u.role).toLowerCase()))) ||
      users.find((u: any) => roles.includes(text(u.role).toLowerCase()));

    return {
      name: possibleName || text(matched?.name) || type,
      code: possibleCode || text(matched?.uniqueCode),
      mobile: possibleMobile || text(matched?.mobile || matched?.phone),
  ...
  const detectType = (raw: any, key: string) => {
    ...
    if (k.includes("machine") || k.includes("machinery") || raw.equipment || raw.machineName || raw.dailyRate || raw.rent) return "Machine Rentals";
    if (k.includes("labour") || raw.trade || raw.workerCount || raw.labourName) return "Labours";
    if (k.includes("realestate") || k.includes("real_estate") || raw.propertyType || raw.propertyName) return "Real Estate";
    if (k.includes("contractor") || raw.contractorName || raw.serviceType === "contractor") return "Contractors";
    return "Products";
  };
  ...
  const normalizeItem = (raw: any, sourceKey: string, index: number): MarketItem | null => {
    ...
    if (skipKeys.some(k => lowerKey.includes(k))) return null;

    const type = detectType(raw, sourceKey);
  ...
  const saveEnquiry = (item: MarketItem) => {
    ...
    const msg = encodeURIComponent(
      `BuildMitra Enquiry\n\nItem: ${item.title}\nUploader: ${item.uploaderName}${item.uploaderCode ? ` (${item.uploaderCode})` : ""}\nQty: ${qty}\nSeen Rate: ₹${item.rate}/${item.unit}\nDelivery: ${deliveryLocation}\nInstruction: ${instructions}\nBuyer: ${buyerName}\nBuyer Mobile: ${buyerMobile}`
    );
    ...
    <div style={{ padding: 20, background: "#f3f4f6", minHeight: "100vh", fontFamily: "Arial" }}>
      ...
      {filtered.length === 0 && (
        <div style={{ background: "white", padding: 30, borderRadius: 12, textAlign: "center", color: "#666", marginTop: 20 }}>
          No listings found. Check uploaded items, selected category, or pincode availability.
        </div>
      )}
