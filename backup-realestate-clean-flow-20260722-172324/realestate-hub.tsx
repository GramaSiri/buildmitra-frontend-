...
export default function RealEstateHub() {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLayouts(
        JSON.parse(localStorage.getItem("bm_realestate_layouts") || "[]")
      );

      setPlots(
        JSON.parse(localStorage.getItem("bm_realestate_plots") || "[]")
      );

      try {
        const res = await fetch(`${API_BASE}/api/realestate`);
        ...
        setProperties(data.properties || data.listings || []);
      } catch (err) {
